...
#else
    m_pInfiniteNode =
            new UCHAR[sizeof(LeafNode) + sizeof(Element) + sizeof(UINT64)];
    memset(m_pInfiniteNode, 0,
           sizeof(LeafNode) + sizeof(Element) + sizeof(UINT64));
    ((LeafNode *)m_pInfiniteNode)->m_nType = 3;
    ((LeafNode *)m_pInfiniteNode)->m_nVertex = 0xFFFFFFFF;
    SET_STORAGE_TAG(((LeafNode *)m_pInfiniteNode)->m_nType, 0);
...
OcTree::~OcTree(void) {
    if (m_pHashMap) delete m_pHashMap;
    if (m_pSingleHashMap) delete m_pSingleHashMap;
