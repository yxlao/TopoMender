    ...
    while (ch > 0) {
        ...
        switch (ch) {
            case 'v':  // vertex
                ch = fgetc(f);
                if ((ch != ' ') && (ch != '\t')) break;
                fscanf(f, "%f%f%f", &tvertex.m_fPosition[0],
                       &tvertex.m_fPosition[1], &tvertex.m_fPosition[2]);
            ...
            case 'f':  // face
                ...
                if ((ch != ' ') && (ch != '\t')) break;
                fscanf(f, "%ld%ld%ld\n", &l[0], &l[1], &l[2]);
                for (int j = 0; j < 3; j++) tface.m_iVertex[j] = l[j] - 1;
                // cal normal
                float vvv[2][3];
                for (int i = 0; i < 3; i++) {
                    vvv[0][i] = m_vVertex[tface.m_iVertex[1]].m_fPosition[i] -
                                m_vVertex[tface.m_iVertex[0]].m_fPosition[i];
                    vvv[1][i] = m_vVertex[tface.m_iVertex[2]].m_fPosition[i] -
                                m_vVertex[tface.m_iVertex[0]].m_fPosition[i];
                }
                vNorm = Vector3D(vvv[0][0], vvv[0][1], vvv[0][2]) ^
                        Vector3D(vvv[1][0], vvv[1][1], vvv[1][2]);
                vNorm.normalize();
...
void RenderMesh::ReCalcNormal() {
    for (int j = 0; j < m_vFace.size(); j++) {
        ...
        float vvv[2][3];
        for (int i = 0; i < 3; i++) {
            vvv[0][i] = m_vVertex[m_vFace[j].m_iVertex[1]].m_fPosition[i] -
                        m_vVertex[m_vFace[j].m_iVertex[0]].m_fPosition[i];
            vvv[1][i] = m_vVertex[m_vFace[j].m_iVertex[2]].m_fPosition[i] -
                        m_vVertex[m_vFace[j].m_iVertex[0]].m_fPosition[i];
        }
        vNorm = Vector3D(vvv[0][0], vvv[0][1], vvv[0][2]) ^
                Vector3D(vvv[1][0], vvv[1][1], vvv[1][2]);
