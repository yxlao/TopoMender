...
bool OcTree::LoadFromSOG(string strSOGFile) {
    ...
    printf("Origin : (%6.4f, %6.4f, %6.4f), Dimension : %6.4f, Level : %d\n",
           m_vCorner.pVec[0], m_vCorner.pVec[1], m_vCorner.pVec[2], m_dbLength,
           m_nMaxLevel);
#endif
        ...
        case 0:
            *ppNode = m_cMemoryManager.New(sizeof(InterNode));
            ...
            for (int i = 0; i < 2; i++)
                for (int j = 0; j < 2; j++)
                    for (int k = 0; k < 2; k++)
                        RecLoadFromSOG(&(((InterNode *)(*ppNode))
                                                 ->m_pNode[i * 4 + j * 2 + k]),
                                       pFile);
            break;
        case 1:
...
void OcTree::RecDuplicateOcTree(UCHAR **ppNode,
                                UCHAR *pOcTreeNode,
                                OcTree *pOcTree) {
...
void OcTree::RecLoadFromOcTree(UCHAR **ppNode,
                               UCHAR *pOcTreeNode,
                               OcTree *pOcTree) {
    UCHAR nType = (*pOcTreeNode) & 0x3;
    InterNode *pTempInter;
            ...
            *ppNode = m_cMemoryManager.New(sizeof(LeafNode) +
            ...
            if (nTemp & 1) {
                *pPointer = 0;
                ...
                if (GET_POINT_INSIDE(nNext, 0) && GET_POINT_INSIDE(nNext, 1) &&
                    GET_POINT_INSIDE(nNext, 2) && GET_POINT_INSIDE(nNext, 3) &&
                    GET_POINT_INSIDE(nNext, 4) && GET_POINT_INSIDE(nNext, 5) &&
                    GET_POINT_INSIDE(nNext, 6) && GET_POINT_INSIDE(nNext, 7)) {
