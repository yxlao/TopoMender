...
void OcTree::RecTraversalFace(UCHAR *pNode[2],
        ...
        switch (m_nType) {
                ...
                if (pThisFace->m_bObject) {
                    for (int i = 0; i < 2; i++)
                        if (!GetElementCell(pNode[i])->m_bObject) {
                            ...
                            GetFacePosition(n64Position, nLevel, dirFace, nI, 0,
                                            n64Temp);
                            dirEdge = (dirFace + 2 - i) % 3;
                            GetEdgeFromPosition(n64Temp, nLevel[nI], dirEdge,
                                                pEdge, n64Edge, nEdge);
                            RecContouringEdge(pNode[i], n64Position[i],
                                              nLevel[i], pEdge, n64Edge, nEdge,
                                              dirEdge, 0, listIndex);

                            GetFacePosition(n64Position, nLevel, dirFace, nI,
                                            1 + i, n64Temp);
            ...
            case 6:
                ...
                if (!pThisFace->m_bSketch && pThisFace->m_bSkeleton) {
                    SkeletonElement element;
                    element.m_n64Size = 0;
                    element.m_n64Position = LeafPosition(
                            n64Position[nI], nLevel[nI],
                            Constant::I_SHIFT_FP[Constant::II_SHIFT_CF[dirFace]
                                                                      [nI]][0]);
...
void OcTree::RecTraversalEdge(UCHAR *pNode[2][2],
    ...
    if ((*(pNode[0][0]) & 0x3) == 0 || (*(pNode[0][1]) & 0x3) == 0 ||
        ...
        switch (m_nType) {
            case 5:
                pThisEdge =
                        GetElementEdge(pNode, nLevel, dirEdge, nI, nJ, nIndex);
                if (pThisEdge->m_bObject) {
                    ...
                    GetEdgePosition(n64Position, nLevel, dirEdge, nI, nJ, 0,
                                    n64ThisEdge[0]);
                    GetEdgePosition(n64Position, nLevel, dirEdge, nI, nJ, 1,
                                    n64ThisEdge[1]);
                    for (int i = 0; i < 2; i++) {
                        listIndex.clear();
                        for (int j = 0; j < 2; j++) {
                        ...
                        if (CheckStorageFace(pFace) &&
                            !GetElementFace(pFace, nFace, dirFace)->m_bObject) {
                            InsertIndex(listIndex,
                                        GetIndex(pFace[i], n64Face[i],
                                                 n64ThisEdge[0]));
                            InsertIndex(listIndex,
                                        GetIndex(pFace[i], n64Face[i],
                                                 n64ThisEdge[1]));
                            InsertIndex(listIndex,
                                        GetIndex(pFace[1 - i], n64Face[1 - i],
                                                 n64ThisEdge[1]));
                            InsertIndex(listIndex,
                                        GetIndex(pFace[1 - i], n64Face[1 - i],
                                                 n64ThisEdge[0]));
            ...
            case 6:
                ...
                if (!pThisEdge->m_bSketch && pThisEdge->m_bSkeleton) {
                    ...
                    element.m_n64Position = LeafPosition(
                            ...
                            Constant::I_SHIFT_EP[Constant::II_SHIFT_CE[dirEdge]
                                                                      [nI][nJ]]
                                                [0]);
...
void OcTree::RecTraversalPoint(UCHAR *pNode[2][2][2],
        ...
        switch (m_nType) {
                ...
                for (int i = 0; i < 2; i++)
                    for (int j = 0; j < 2; j++)
                        for (int k = 0; k < 2; k++) {
                            ...
                            Element *pItCell = (Element *)(pNode[i][j][k] +
                                                           sizeof(LeafNode));
                            if (pThisPoint->m_bObject &&
                                pItCell->m_bObject == 0 &&
                                IS_CELL_IRREGULAR(pItNode->m_nVertex)) {
                                UINT64 n64ThisPoint =
                                ...
                                if (bVertex) {
                                    if (m_nContourPass == 2) {
                                        ...
                                        float fPos[3];
                                        if (pThisPoint->m_bSketch == 1 &&
                                            pThisPoint->m_cSimpleTag == 1) {
                                            fPos[0] =
                                                    (float)(m_dbNodeLength *
                                                                    (INT64GETI(
                                                                             n64ThisPoint) +
                                                                     SKELETON_WIDTH_DOUBLE *
                                                                             (i * 2 -
                                                                              1) *
                                                                             NLENGTH(m_nMaxLevel +
                                                                                     1)) +
            ...
            case 5:
                ...
                if (pThisPoint->m_bObject) {
                    ...
                    list<int> listIndex;
                    UINT64 n64ThisPoint;
                    GetPointPosition(n64Position, nLevel, nI, nJ, nK,
                                     n64ThisPoint);
                    ...
                    for (int i = 0; i < 2; i++) {
                        ...
                        if (CheckStorageEdge(pEdge) &&
                            !GetElementEdge(pEdge, nEdge, dirEdge)->m_bObject) {
                            InsertIndex(listIndex,
                                        GetIndex(pEdge[0][0], n64Edge[0][0],
                                                 n64ThisPoint));
                            InsertIndex(listIndex, GetIndex(pEdge[i][1 - i],
                                                            n64Edge[i][1 - i],
                                                            n64ThisPoint));
                            InsertIndex(listIndex,
                                        GetIndex(pEdge[1][1], n64Edge[1][1],
                                                 n64ThisPoint));
                            InsertIndex(listIndex, GetIndex(pEdge[1 - i][i],
                                                            n64Edge[1 - i][i],
                                                            n64ThisPoint));
                            if (listIndex.size() >= 3) {
                                if (m_nContourPass == 2)
            ...
            case 6:
                ...
                if (!pThisPoint->m_bSketch && pThisPoint->m_bSkeleton) {
                    ...
                    element.m_n64Size = 0;
                    element.m_n64Position = LeafPosition(
                            n64Position[nI][nJ][nK], nLevel[nI][nJ][nK],
...
void OcTree::RecTraversalCell(UCHAR *pNode, int nLevel) {
    if (NOT_LEAF(pNode)) {
        ...
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++)
                for (int k = 0; k < 2; k++)
                    Leaf(pNode, nLevel, i, j, k, pNodePoint[i][j][k],
                         nPoint[i][j][k]);
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++)
                for (int k = 0; k < 2; k++) {
        ...
        switch (m_nType) {
            ...
            case 2:
                pThisCell = GetElementCell(pNode);
                if (pThisCell->m_bObject) m_nCountCell++;
                break;
            case 3:
                pThisCell = GetElementCell(pNode);
                pThisNode = (LeafNode *)pNode;
...
void OcTree::RecTraversalFace(UCHAR *pNode[2], int nLevel[2], int dirFace) {
    if ((*(pNode[0]) & 0x3) == 0 || (*(pNode[1]) & 0x3) == 0) {
        ...
        switch (dirFace) {
            case 0:
                for (int i = 0; i < 2; i++)
                    for (int j = 0; j < 2; j++)
                        for (int k = 0; k < 2; k++)
                            Leaf(pNode[i], nLevel[i], 1 - i, j, k,
                                 pNodePoint[i][j][k], nPoint[i][j][k]);
                for (int i = 0; i < 2; i++)
                    for (int j = 0; j < 2; j++) {
                        for (int k = 0; k < 2; k++) {
                            pNodeFace[k] = pNodePoint[k][i][j];
        ...
        switch (m_nType) {
            ...
            case 2:
                pThisFace = GetElementFace(pNode, nLevel, dirFace, nI, nIndex);
                if (pThisFace->m_bObject) m_nCountFace++;
                break;
            case 3:
                pThisFace = GetElementFace(pNode, nLevel, dirFace, nI, nIndex);
                if (pThisFace->m_bObject) {
                    for (int i = 0; i < 2; i++)
                        if (nLevel[i] == nLevel[nI])
                            SET_INSIDE_TAG(((LeafNode *)pNode[i])->m_nVertex,
                                           dirFace * 2 + (1 - i) + 1);
                }
                break;
            case 256:
            ...
            case 256 + 1:
                ...
                if (!pThisFace->m_bSkeleton && pThisFace->m_cPointer < 2) {
                    ...
                    if (pTempCell->m_bSkeleton ||
                        pTempCell->m_cPointer != dirFace * 2 + (1 - i))
                        m_bCorrect = false;
                }
                break;
        }
    ...
    if ((*(pNode[0][0]) & 0x3) == 0 || (*(pNode[0][1]) & 0x3) == 0 ||
        ...
        switch (dirEdge) {
            case 0:
                for (int i = 0; i < 2; i++)
                    for (int j = 0; j < 2; j++)
                        for (int k = 0; k < 2; k++)
                            Leaf(pNode[j][k], nLevel[j][k], i, 1 - j, 1 - k,
                                 pNodePoint[i][j][k], nPoint[i][j][k]);
                for (int i = 0; i < 2; i++) {
                    for (int j = 0; j < 2; j++)
                        for (int k = 0; k < 2; k++) {
            ...
            case 1:
                for (int i = 0; i < 2; i++)
                    for (int j = 0; j < 2; j++)
                        for (int k = 0; k < 2; k++)
                            Leaf(pNode[k][i], nLevel[k][i], 1 - i, j, 1 - k,
                                 pNodePoint[i][j][k], nPoint[i][j][k]);
                for (int i = 0; i < 2; i++) {
                    for (int j = 0; j < 2; j++)
                        for (int k = 0; k < 2; k++) {
            ...
            case 2:
                for (int i = 0; i < 2; i++)
                    for (int j = 0; j < 2; j++)
                        for (int k = 0; k < 2; k++)
                            Leaf(pNode[i][j], nLevel[i][j], 1 - i, 1 - j, k,
                                 pNodePoint[i][j][k], nPoint[i][j][k]);
                for (int i = 0; i < 2; i++) {
                    for (int j = 0; j < 2; j++)
                        for (int k = 0; k < 2; k++) {
        ...
        switch (m_nType) {
            ...
            case 2:
                pThisEdge =
                        GetElementEdge(pNode, nLevel, dirEdge, nI, nJ, nIndex);
                if (pThisEdge->m_bObject) m_nCountEdge++;
                break;
            case 3:
                pThisEdge =
                        GetElementEdge(pNode, nLevel, dirEdge, nI, nJ, nIndex);
            ...
            case 256 + 1:
                ...
                if (!pThisEdge->m_bSkeleton && pThisEdge->m_cPointer < 4) {
                    ...
                    if (CheckStorageFace(pFace)) {
                        Element *pTempFace =
                                GetElementFace(pFace, nFace, dirFace);
                        if (pTempFace->m_bSkeleton ||
                            pTempFace->m_cPointer != (c * 2) + (1 - i) + 2)
                            m_bCorrect = false;
                    }
                }
                break;
...
void OcTree::RecTraversalPoint(UCHAR *pNode[2][2][2], int nLevel[2][2][2]) {
    if ((*(pNode[0][0][0]) & 0x3) == 0 || (*(pNode[0][0][1]) & 0x3) == 0 ||
        ...
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++)
                for (int k = 0; k < 2; k++)
                    Leaf(pNode[i][j][k], nLevel[i][j][k], 1 - i, 1 - j, 1 - k,
                         pNodePoint[i][j][k], nPoint[i][j][k]);
        RecTraversalPoint(pNodePoint, nPoint);
    } else {
        int nI, nJ, nK, nIndex;
        ...
        switch (m_nType) {
            ...
            case 2:
                pThisPoint = GetElementPoint(pNode, nLevel, nI, nJ, nK, nIndex);
                if (pThisPoint->m_bObject) m_nCountPoint++;
                break;
            case 3:
                pThisPoint = GetElementPoint(pNode, nLevel, nI, nJ, nK, nIndex);
                if (pThisPoint->m_bObject) {
                        ...
                        for (int j = 0; j < 2; j++)
                            for (int k = 0; k < 2; k++)
                                if (pNode[i][j][k] != pNode[1 - i][j][k] &&
                                    ...
                                    SET_INSIDE_TAG(((LeafNode *)pNode[i][j][k])
                                                           ->m_nVertex,
                                                   (1 - i) * 4 + (1 - j) * 2 +
                                                           (1 - k) + 1 + 6 +
                                                           12);
            ...
            case 256 + 1:
                ...
                if (!pThisPoint->m_bSkeleton) {
                    ...
                    if (CheckStorageEdge(pEdge)) {
                        Element *pTempEdge =
                                GetElementEdge(pEdge, nEdge, dirEdge);
                        if (pTempEdge->m_bSkeleton ||
                            pTempEdge->m_cPointer != 1 - i + 4)
                            m_bCorrect = false;
                    }
                }
                break;
