...
bool OcTree::RecThinCell(UCHAR *pNode, UINT64 n64Position, int nLevel) {
    ...
    if (NOT_LEAF(pNode)) {
        ...
        // !pThisCell->m_bSketch) { 	if (pThisCell->m_cSimpleTag ==
        //m_nTestLastOut) { 		pThisCell->m_bSkeleton = 0; 		pThisCell->m_cSimpleTag
        //= 0;
        //	}
        //}
    }
...
bool OcTree::RecThinFace(UCHAR *pNode[2],
        ...
        if (pThisFace->m_bObject && pThisFace->m_bSkeleton &&
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if (pTempCell->m_bObject && pTempCell->m_bSkeleton) {
                    ...
                    if (nSimpleTest == 1) {
                        ...
                        nFace2Cell = i;
                        n64WitnessElement = GetCellCenterPosition(
                                n64Position[i], nLevel[i]);
                    } else {
                        break;
                    }
                }
            ...
            if (nSimpleTest == 1) {
                if (m_bCountOnly) {
                    ...
                    // set face
                    m_n64SimpleElement =
                            GetFaceCenterPosition(n64Position, nLevel, dirFace);
                    ...
                    if (*pThat == INT64_INVALID) {
                        *pThat = *pThis;
                        ...
                        if (Distance(pThis, &n64WitnessElement) <
                            Distance(pThat, &n64WitnessElement)) {
                            *pThat = *pThis;
                        }
                    }
                } else {
                    if (*GetFootPoint(pThisFace) != INT64_INVALID &&
                        *GetFootPoint(pUniqueCell) ==
                                *GetFootPoint(pThisFace) &&
                        Distance(GetFootPoint(pUniqueCell),
                                 &n64WitnessElement) < m_n64Threshold) {
                        // printf("Face : (%16I64x, %d)\n",
                        // GetFaceCenterPosition(n64Position, nLevel, dirFace),
                        // dirFace); printf("Cell (witness) : (%16I64x)\n",
                        // n64WitnessElement);
...
bool OcTree::RecThinEdge(UCHAR *pNode[2][2],
        ...
        if (pThisEdge->m_bObject && pThisEdge->m_bSkeleton &&
                ...
                if (CheckStorageFace(pFace)) {
                    ...
                    if (pTempFace->m_bObject && pTempFace->m_bSkeleton) {
                        ...
                        if (nSimpleTest == 1) {
                            ...
                            nEdge2Face = (0 * 2) + i;
                            n64WitnessElement = GetFaceCenterPosition(
                                    n64Face, nFace, dirFace);
                        } else {
                            break;
                        }
                    }
            ...
            if (nSimpleTest == 1) {
                if (m_bCountOnly) {
                    ...
                    // set edge
                    m_n64SimpleElement =
                            GetEdgeCenterPosition(n64Position, nLevel, dirEdge);
                    ...
                    if (*GetFootPoint(pThisEdge) != INT64_INVALID &&
                        *GetFootPoint(pUniqueFace) ==
                                *GetFootPoint(pThisEdge) &&
                        Distance(GetFootPoint(pUniqueFace),
                                 &n64WitnessElement) < m_n64Threshold) {
                        // printf("Edge : (%16I64x, %d)\n",
                        // GetEdgeCenterPosition(n64Position, nLevel, dirEdge),
                        // dirEdge); printf("Face (witness) : (%16I64x, %d)\n",
...
bool OcTree::RecThinPoint(UCHAR *pNode[2][2][2],
    ...
    if (NOT_LEAF(pNode[0][0][0]) || NOT_LEAF(pNode[0][0][1]) ||
        NOT_LEAF(pNode[0][1][0]) || NOT_LEAF(pNode[0][1][1]) ||
        NOT_LEAF(pNode[1][0][0]) || NOT_LEAF(pNode[1][0][1]) ||
        NOT_LEAF(pNode[1][1][0]) || NOT_LEAF(pNode[1][1][1])) {
        UCHAR *pNodePoint[2][2][2];
        UINT64 n64Point[2][2][2];
        int nPoint[2][2][2];
        for (int i = 0; i < 2; i++)
                ...
                if (CheckStorageEdge(pEdge)) {
                    ...
                    if (pTempEdge->m_bObject && pTempEdge->m_bSkeleton) {
                        ...
                        if (nSimpleTest == 1) {
                            ...
                            n64WitnessElement = GetEdgeCenterPosition(
                                    n64Edge, nEdge, dirEdge);
                        } else {
                            break;
                        }
                    }
            ...
            if (nSimpleTest == 1) {
                if (m_bCountOnly) {
                    ...
                    if (*GetFootPoint(pThisPoint) != INT64_INVALID &&
                        *GetFootPoint(pUniqueEdge) ==
                                *GetFootPoint(pThisPoint) &&
                        Distance(GetFootPoint(pUniqueEdge),
                                 &n64WitnessElement) < m_n64Threshold) {
                        // printf("Edge : (%16I64x)\n",
                        // GetPointCenterPosition(n64Position, nLevel));
