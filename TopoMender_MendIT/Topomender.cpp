...
Topomender::Topomender(string meshStr,
                       string sogStr,
                       string sktStr,
                       string outputStr) {
...
bool Topomender::testValid() {
    if (!(m_strMeshFile.substr(m_strMeshFile.length() - 4) == ".obj"))
        return false;
    if (!(m_strSOGFile.substr(m_strSOGFile.length() - 4) == ".sog"))
        return false;
    if (!(m_strSktFile.substr(m_strSktFile.length() - 4) == ".axs"))
        return false;
    if (!(m_strOutputFile.substr(m_strOutputFile.length() - 4) == ".obj"))
        return false;
    return true;
}
