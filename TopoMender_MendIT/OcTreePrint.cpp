...
void OcTree::PrintSkeletonInfo() {
    ...
    printf("Inside skeleton contains %d cells, %d faces, %d edges, %d "
           "points.\n",
           m_nCountCell, m_nCountFace, m_nCountEdge, m_nCountPoint);
    printf("Total %d handles.\n",
           m_nCountEdge + m_nCountCell - m_nCountFace - m_nCountPoint + 1);
    ...
    printf("Outside skeleton contains %d cells, %d faces, %d edges, %d "
           "points.\n",
           m_nCountCell, m_nCountFace, m_nCountEdge, m_nCountPoint);
    printf("Total %d handles.\n",
           -m_nCountEdge - m_nCountCell + m_nCountFace + m_nCountPoint + 1);
...
void OcTree::PrintPureSkeletonInfo() {
    ...
    for (list<SkeletonElement>::iterator it = m_listSkeleton.begin();
        ...
        if (it->m_pElement->m_bSkeleton) nCount[it->m_nDimension]++;
    }

    printf("Inside pure skeleton contains %d cells, %d faces, %d edges, %d "
           "points.\n",
    ...
    for (list<SkeletonElement>::iterator it = m_listDualSkeleton.begin();
        ...
        if (it->m_pElement->m_bSkeleton) nOutCount[it->m_nDimension]++;
    }

    printf("Outside pure skeleton contains %d cells, %d faces, %d edges, %d "
           "points.\n",
           nOutCount[3], nOutCount[2], nOutCount[1], nOutCount[0]);

    printf("Handle numbers are : inside = %d, outside = %d\n",
           m_nSketchEdge + m_nSketchCell - m_nSketchFace - m_nSketchPoint + 1 +
                   nCount[1] + nCount[3] - nCount[2] - nCount[0],
