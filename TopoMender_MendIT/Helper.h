...
typedef struct _ELEMENT {
    ...
    UCHAR m_cPointer : 3;  // c: 0-5 f;  f: 0-1 c, 2-5 e; e: 0-3 f, 4-5 p; p:
                           // 0-5 e
} Element;
...
#define INT64GETIJK(n, x, y, z) \
    (x = (int)INT64GETI(n), y = (int)INT64GETJ(n), z = (int)INT64GETK(n))
...
#define NODE_IN_NODE(n, l, m)                    \
    (INT64GETI(m) >= INT64GETI(n) &&             \
     INT64GETI(m) < INT64GETI(n) + NLENGTH(l) && \
     INT64GETJ(m) >= INT64GETJ(n) &&             \
     INT64GETJ(m) < INT64GETJ(n) + NLENGTH(l) && \
     INT64GETK(m) >= INT64GETK(n) && INT64GETK(m) < INT64GETK(n) + NLENGTH(l))
