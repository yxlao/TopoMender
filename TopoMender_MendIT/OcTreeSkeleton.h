...
typedef struct _SKELETON_ELEMENT {
    ...
    _SKELETON_ELEMENT(UINT64 n64Position,
                      int nLevel,
                      int nDir,
                      int nDimension,
                      Element *pElement)
...
public:
    bool operator()(SkeletonElement &val) {
        return (val.m_pElement->m_bSkeleton == 0 ||
                val.m_pElement->m_bObject == 0);
    }
};

class is_removed_dual_skeleton
    : public std::unary_function<SkeletonElement, bool> {
public:
    bool operator()(SkeletonElement &val) {
        return (val.m_pElement->m_bSkeleton == 0 ||
                val.m_pElement->m_bObject == 1);
    }
};
