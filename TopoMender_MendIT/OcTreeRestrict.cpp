...
void OcTree::LevelRestrict() {
    ...
    printf("Restrict nodes (neighboring nodes differ by no more than one "
           "level), finished.\n");
#endif
}
    ...
    if (NOT_LEAF(pNode)) {
        ...
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++) {
                ...
                if (RecLevelRestrictCheckFace(pNodeFace, nFace, 0)) bRtn = true;
                for (int k = 0; k < 2; k++) {
                    pNodeFace[k] = pNodePoint[j][k][i];
                    nFace[k] = nPoint[j][k][i];
                }
                if (RecLevelRestrictCheckFace(pNodeFace, nFace, 1)) bRtn = true;
                for (int k = 0; k < 2; k++) {
                    pNodeFace[k] = pNodePoint[i][j][k];
                    nFace[k] = nPoint[i][j][k];
                }
                if (RecLevelRestrictCheckFace(pNodeFace, nFace, 2)) bRtn = true;
            }
    }
...
bool OcTree::RecLevelRestrictCheckFace(UCHAR *pNode[2],
                                       int nLevel[2],
                                       int dirFace) {
    bool bRtn = false;
...
void OcTree::RecLevelRestrictNode(UCHAR **ppNode) {
    if (NOT_LEAF(*ppNode)) {
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++)
                for (int k = 0; k < 2; k++) {
                    RecLevelRestrictNode(&((InterNode *)(*ppNode))
                                                  ->m_pNode[i * 4 + j * 2 + k]);
                }
    } else {
        if (((NodeHeader *)(*ppNode))->m_cType == 1 &&
            ((NodeHeader *)(*ppNode))
                    ->m_bLastTime) {  // well, we should split it
            ...
            for (int i = 0; i < 2; i++)
                for (int j = 0; j < 2; j++)
                    for (int k = 0; k < 2; k++) {
                        ((InterNode *)(*ppNode))->m_pNode[i * 4 + j * 2 + k] =
                                m_cMemoryManager.New(sizeof(LeafNode));
                        ((LeafNode *)((InterNode *)(*ppNode))
                                 ->m_pNode[i * 4 + j * 2 + k])
                                ->m_nType = ((LeafNode *)(pOldNode))->m_nType;
                        ((LeafNode *)((InterNode *)(*ppNode))
                                 ->m_pNode[i * 4 + j * 2 + k])
                                ->m_nVertex =
