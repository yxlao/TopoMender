...
void OcTree::CheckEdge(UINT64 n64CheckEdge, int nCheckEdge, int dirCheckEdge) {
    ...
    int nEdge[2][2];
    int nPoint[2][2][2];
    RecCheckEdgeCell(m_pInfiniteNode, m_n64Infinite, -1, n64CheckEdge,
    ...
    for (int i = 0; i < 2; i++) {
        ...
        n64Face[i] = 0;
        nFace[i] = 0;
        RecCheckEdgeFace(pNodeFace, n64Face, nFace, 0, n64CheckEdge, nCheckEdge,
                         dirCheckEdge);
        RecCheckEdgeFace(pNodeFace, n64Face, nFace, 1, n64CheckEdge, nCheckEdge,
                         dirCheckEdge);
        RecCheckEdgeFace(pNodeFace, n64Face, nFace, 2, n64CheckEdge, nCheckEdge,
                         dirCheckEdge);
    }
    for (int i = 0; i < 2; i++)
        for (int j = 0; j < 2; j++) {
            ...
            n64Edge[i][j] = 0;
            nEdge[i][j] = 0;
            RecCheckEdgeEdge(pNodeEdge, n64Edge, nEdge, 0, n64CheckEdge,
                             nCheckEdge, dirCheckEdge);
            RecCheckEdgeEdge(pNodeEdge, n64Edge, nEdge, 1, n64CheckEdge,
                             nCheckEdge, dirCheckEdge);
            RecCheckEdgeEdge(pNodeEdge, n64Edge, nEdge, 2, n64CheckEdge,
                             nCheckEdge, dirCheckEdge);
        }
    for (int i = 0; i < 2; i++)
        for (int j = 0; j < 2; j++)
            for (int k = 0; k < 2; k++) {
                ...
                n64Point[i][j][k] = 0;
                nPoint[i][j][k] = 0;
                RecCheckEdgePoint(pNodePoint, n64Point, nPoint, n64CheckEdge,
                                  nCheckEdge, dirCheckEdge);
            }
}

void OcTree::RecCheckEdgeCell(UCHAR *pNode,
                              UINT64 n64Position,
                              int nLevel,
                              UINT64 n64CheckEdge,
                              int nCheckEdge,
                              int dirCheckEdge) {
    if (!CheckEdgeForCell(n64Position, nLevel, n64CheckEdge, nCheckEdge,
                          dirCheckEdge))
        return;
        ...
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++)
                for (int k = 0; k < 2; k++) {
                    ...
                    nCell = nPoint[i][j][k];
                    RecCheckEdgeCell(pNodeCell, n64Cell, nCell, n64CheckEdge,
                                     nCheckEdge, dirCheckEdge);
                }
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++) {
                ...
                RecCheckEdgeFace(pNodeFace, n64Face, nFace, 0, n64CheckEdge,
                                 nCheckEdge, dirCheckEdge);
                for (int k = 0; k < 2; k++) {
                ...
                RecCheckEdgeFace(pNodeFace, n64Face, nFace, 1, n64CheckEdge,
                                 nCheckEdge, dirCheckEdge);
                for (int k = 0; k < 2; k++) {
                ...
                RecCheckEdgeFace(pNodeFace, n64Face, nFace, 2, n64CheckEdge,
                                 nCheckEdge, dirCheckEdge);
            }
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 2; j++)
...
void OcTree::RecCheckEdgeFace(UCHAR *pNode[2],
    ...
    if (!CheckEdgeForFace(n64ThisFace, nThisFace, dirFace, n64CheckEdge,
                          nCheckEdge, dirCheckEdge))
        return;
            ...
            case 0:
                ...
                for (int i = 0; i < 2; i++)
                    for (int j = 0; j < 2; j++) {
                        ...
                        RecCheckEdgeFace(pNodeFace, n64Face, nFace, 0,
                                         n64CheckEdge, nCheckEdge,
                                         dirCheckEdge);
                    }
                for (int i = 0; i < 2; i++) {
                    for (int j = 0; j < 2; j++)
            ...
            case 1:
                ...
                for (int i = 0; i < 2; i++)
                    for (int j = 0; j < 2; j++) {
                        ...
                        RecCheckEdgeFace(pNodeFace, n64Face, nFace, 1,
                                         n64CheckEdge, nCheckEdge,
                                         dirCheckEdge);
                    }
                for (int i = 0; i < 2; i++) {
            ...
            case 2:
                ...
                for (int i = 0; i < 2; i++)
                    for (int j = 0; j < 2; j++) {
                        ...
                        RecCheckEdgeFace(pNodeFace, n64Face, nFace, 2,
                                         n64CheckEdge, nCheckEdge,
                                         dirCheckEdge);
                    }
                for (int i = 0; i < 2; i++) {
...
void OcTree::RecCheckEdgeEdge(UCHAR *pNode[2][2],
    ...
    if (!CheckEdgeForEdge(n64ThisEdge, nThisEdge, dirEdge, n64CheckEdge,
                          nCheckEdge, dirCheckEdge))
        return;
...
void OcTree::RecCheckEdgePoint(UCHAR *pNode[2][2][2],
    ...
    if (!CheckEdgeForPoint(n64ThisPoint, n64CheckEdge, nCheckEdge,
                           dirCheckEdge))
        return;
                ...
                if (!pThisPoint->m_bObject && pThisPoint->m_bSkeleton) {
                    ...
                    if (m_nBelong == 1) {
                        ...
                        if (itTemp != m_hashDualSkeleton.end()) {
                            ...
                            UINT64 n64Test[2][2];
                            int nTest[2][2];
                            GetEdgeFromPosition(n64CheckEdge, nCheckEdge,
                                                dirCheckEdge, pTest, n64Test,
                                                nTest);
                            int iTest;
                            switch (dirCheckEdge) {
                                case 0:
                                    if (n64Test[0][0] == n64Position[0][0][0] &&
                                        n64Test[0][1] == n64Position[0][0][1] &&
                                        n64Test[1][0] == n64Position[0][1][0] &&
                                        n64Test[1][1] == n64Position[0][1][1]) {
                                        iTest = 0;
                                    } else {
                                        iTest = 1;
                                    }
                                    m_pBelong->m_cPointer =
                                            dirCheckEdge * 2 + iTest;
                                    m_pBelong->m_cBackPointer = 1 - iTest + 4;
                                    break;
                                case 1:
                                    if (n64Test[0][0] == n64Position[0][0][0] &&
                                        n64Test[0][1] == n64Position[1][0][0] &&
                                        n64Test[1][0] == n64Position[0][0][1] &&
                                        n64Test[1][1] == n64Position[1][0][1]) {
                                        iTest = 0;
                                    } else {
                                        iTest = 1;
                                    }
                                ...
                                case 2:
                                    if (n64Test[0][0] == n64Position[0][0][0] &&
                                        n64Test[0][1] == n64Position[0][1][0] &&
                                        n64Test[1][0] == n64Position[1][0][0] &&
                                        n64Test[1][1] == n64Position[1][1][0]) {
                                        iTest = 0;
                                    } else {
                                        iTest = 1;
                                    }
