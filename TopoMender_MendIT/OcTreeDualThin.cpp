    ...
    for (int i = 0; i < 2; i++)
        for (int j = 0; j < 2; j++)
            for (int k = 0; k < 2; k++) {
                ...
                nPoint[i][j][k] = 0;
                if (RecDualThinPoint(pNodePoint, nPoint)) bRtn = true;
            }
    for (int i = 0; i < 2; i++)
        for (int j = 0; j < 2; j++) {
            for (int ii = 0; ii < 2; ii++)
            ...
            nEdge[i][j] = 0;
            if (RecDualThinEdge(pNodeEdge, nEdge, 0)) bRtn = true;
            if (RecDualThinEdge(pNodeEdge, nEdge, 1)) bRtn = true;
            if (RecDualThinEdge(pNodeEdge, nEdge, 2)) bRtn = true;
        }
    for (int i = 0; i < 2; i++) {
        for (int ii = 0; ii < 2; ii++) {
        ...
        pNodeFace[i] = m_pRoot;
        nFace[i] = 0;
        if (RecDualThinFace(pNodeFace, nFace, 0)) bRtn = true;
        if (RecDualThinFace(pNodeFace, nFace, 1)) bRtn = true;
        if (RecDualThinFace(pNodeFace, nFace, 2)) bRtn = true;
    }
    if (RecDualThinCell(m_pRoot, 0)) bRtn = true;
    return bRtn;
}
            ...
            for (int i = 0; i < 2; i++)
                for (int j = 0; j < 2; j++)
                    for (int k = 0; k < 2; k++) {
                        ...
                        nCell = nPoint[i][j][k];
                        if (RecDualThinCell(pNodeCell, nCell)) bRtn = true;
                    }
        }
    } else {
...
bool OcTree::RecDualThinEdge(UCHAR *pNode[2][2],
                             int nLevel[2][2],
                             int dirEdge) {
    bool bRtn = false;
                ...
                for (int i = 0; i < 2; i++) {
                    ...
                    if (GET_LAST_FIX(pFace[0]) && GET_LAST_FIX(pFace[1])) {
                        if (CheckStorageFace(pFace)) {
                            ...
                            if (!pTempFace->m_bObject &&
                                pTempFace->m_bSkeleton &&
                                pTempFace->m_cSimpleTag != 2) {
    ...
    if (NOT_LEAF(pNode[0][0][0]) || NOT_LEAF(pNode[0][0][1]) ||
        ...
        if (GET_LAST_FIX(pNode[0][0][0]) || GET_LAST_FIX(pNode[0][0][1]) ||
            ...
            for (int i = 0; i < 2; i++)
                for (int j = 0; j < 2; j++)
                    for (int k = 0; k < 2; k++)
                        Leaf(pNode[i][j][k], nLevel[i][j][k], 1 - i, 1 - j,
                             1 - k, pNodePoint[i][j][k], nPoint[i][j][k]);
            if (RecDualThinPoint(pNodePoint, nPoint)) bRtn = true;
        }
    } else {
        ...
        if (!pThisPoint->m_bObject && pThisPoint->m_bSkeleton) {
            ...
            if (m_bCountOnly) {
                for (int i = 0; i < 2; i++) {
                    ...
                    dirEdge = 0;
                    if (GET_LAST_FIX(pEdge[0][0]) &&
                        GET_LAST_FIX(pEdge[0][1]) &&
                        GET_LAST_FIX(pEdge[1][0]) &&
                        GET_LAST_FIX(pEdge[1][1])) {
                        if (CheckStorageEdge(pEdge)) {
                            Element *pTempEdge =
                                    GetElementEdge(pEdge, nEdge, dirEdge);
                            if (!pTempEdge->m_bObject &&
                                pTempEdge->m_bSkeleton &&
                                pTempEdge->m_cSimpleTag != 2) {
