...
double Vector2D::length2() { return x * x + y * y; }
...
bool Vector2D::OnLine(Vector2D v0, Vector2D v1) {
    if (AtWhere(v0, v1) == 0) return true;
    return false;
}

bool Vector2D::Intersect(Vector2D v1,
                         Vector2D v2)  // intersect with uint circle
{
    Vector2D vOrigin;
...
bool Vector2D::Intersect(Vector2D v1,
                         Vector2D v2,
                         Vector2D v3,
                         Vector2D v4)  // tow line intersect
{
    double d = (v4.y - v3.y) * (v1.x - v2.x) - (v2.y - v1.y) * (v3.x - v4.x);
    if (EQUAL_ZERO(d)) return false;
...
double Vector2D::GetArea(Vector2D v) { return x * v.y - v.x * y; }
...
double Vector3D::length() { return sqrt(x * x + y * y + z * z); }

double Vector3D::length2() { return x * x + y * y + z * z; }
