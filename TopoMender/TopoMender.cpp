...
void PrintHelp() {
    ...
    printf("\n");
    printf("Usage :   TopoMender [-skt] SOGFile ThinThresh GrowThresh "
           "ModelFile\n");
    printf("Example : TopoMender eight.sog 1.0 1.0 eight.ply\n");
    printf("\n");
    printf("SOGFile :    Input file, must be of SOG file format\n");
    printf("ModelFile :  Output file, could be of either OBJ or PLY file "
           "format\n");
    printf("ThinThresh : A threshold used in thinning process\n");
    printf("GrowThresh : A threshold used in growing process\n");
    printf("-skt :       Indicates a corresponding SKT file would be "
           "generated\n");
    printf("\n");
}

bool ParseSOGFile(
        FILE *pFile, double &dbX, double &dbY, double &dbZ, double &dbSize) {
    char pHeadBuf[128];
...
bool FindString(int nMin, int nMax, char *argv[], const char *strFind) {
    for (int i = nMin; i <= nMax; i++)
        if (stricmp(argv[i], strFind) == 0) return true;
    return false;
}
...
int _tmain(int argc, char *argv[]) {
    ...
    // -h and -help
    if (argc < 5 || FindString(1, argc - 5, argv, "-HELP") ||
        FindString(1, argc - 5, argv, "-H")) {
        PrintHelp();
        return 0;
    }
