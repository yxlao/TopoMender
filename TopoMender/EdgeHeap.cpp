...
CEdgeHeap::~CEdgeHeap(void) { delete[] m_pElement; }
...
bool CEdgeHeap::Push(UCHAR* pNode[2][2],
    ...
    m_pElement[iIndex] =
            CEdgeHeapElement(pNode, n64Position, nLevel, dirEdge, fValue);
...
CEdgeHeapElement CEdgeHeap::Pop() {
    ...
    while (true) {
        ...
        if (iChildLeft >= m_nNum) {
            ...
            if (m_pElement[iChildLeft].m_fValue >=
                m_pElement[iChildRight].m_fValue) {
                if (fValue < m_pElement[iChildLeft].m_fValue) {
                    m_pElement[iIndex] = m_pElement[iChildLeft];
                    iIndex = iChildLeft;
                } else {
...
bool CEdgeHeap::Empty() { return (m_nNum == 0); }
