...
#define INT64TRANS(i, j, k) \
    (((UINT64)(i) << 40) | ((UINT64)(j) << 20) | ((UINT64)(k)))
#define INT64GETI(n) (((n) >> 40) & (0xfffff))
#define INT64GETJ(n) (((n) >> 20) & (0xfffff))
#define INT64GETK(n) ((n) & (0xfffff))
#define INT64TRANSD(i, j, k, d)                                        \
    (((UINT64)(d) << 60) | ((UINT64)(i) << 40) | ((UINT64)(j) << 20) | \
     ((UINT64)(k)))
...
#define POSITION_TRANS_X(n) \
    (0.5 * m_dbNodeLength * (INT64GETI(n)) + m_vCorner.pVec[0])
#define POSITION_TRANS_Y(n) \
    (0.5 * m_dbNodeLength * (INT64GETJ(n)) + m_vCorner.pVec[1])
#define POSITION_TRANS_Z(n) \
    (0.5 * m_dbNodeLength * (INT64GETK(n)) + m_vCorner.pVec[2])
#define POSITION_TRANS(n) \
    (CVector3D(POSITION_TRANS_X(n), POSITION_TRANS_Y(n), POSITION_TRANS_Z(n)))
...
public:
    void PrintNodeInfo(UCHAR *pNode) {
        ...
        printf("\n(%16I64x, %16I64x, %d)", pTempNode->m_n64Type,
               pTempNode->m_n64SimpleTag, pTempNode->m_nVertex);
    }
    void PrintNodeInfo(UCHAR *pNode, UINT64 n64Position, int nLevel) {
        LEAF_NODE *pTempNode = (LEAF_NODE *)pNode;
        printf("\n(%16I64x, %16I64x, %d) @ %I64x ~ %d", pTempNode->m_n64Type,
               pTempNode->m_n64SimpleTag, pTempNode->m_nVertex, n64Position,
               nLevel);
    }
    void PrintCellInfo(UCHAR *pNode, UINT64 n64Position, int nLevel) {
        LEAF_NODE *pTempNode = (LEAF_NODE *)pNode;
        printf("\nCell's info : I/O Tag = %I64d, Simple Tag = %I64d, and "
               "details below :",
               GET_CELL_INSIDE(pTempNode->m_n64Type, 0),
               GET_CELL_SIMPLE(pTempNode->m_n64SimpleTag, 0));
        PrintNodeInfo(pNode, n64Position, nLevel);
    }
    void PrintFaceInfo(UCHAR *pNode[2],
        ...
        printf("\nFace's info : I/O Tag = %I64d, Simple Tag = %I64d, dirFace = "
               "%d, and details below :",
               GET_FACE_INSIDE(pTempNode->m_n64Type, nIndex),
               GET_FACE_SIMPLE(pTempNode->m_n64SimpleTag, nIndex), dirFace);
        for (int i = 0; i < 2; i++)
            PrintNodeInfo(pNode[i], n64Position[i], nLevel[i]);
    }
    void PrintEdgeInfo(UCHAR *pNode[2][2],
        ...
        printf("\nEdge's info : I/O Tag = %I64d, Simple Tag = %I64d, dirEdge = "
               "%d, and details below :",
               GET_EDGE_INSIDE(pTempNode->m_n64Type, nIndex),
               GET_EDGE_SIMPLE(pTempNode->m_n64SimpleTag, nIndex), dirEdge);
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++)
                PrintNodeInfo(pNode[i][j], n64Position[i][j], nLevel[i][j]);
    }
    void PrintPointInfo(UCHAR *pNode[2][2][2],
                        UINT64 n64Position[2][2][2],
                        int nLevel[2][2][2]) {
        int nI, nJ, nK, nIndex;
        GetStoragePoint(nLevel, nI, nJ, nK, nIndex);
        LEAF_NODE *pTempNode = (LEAF_NODE *)pNode[nI][nJ][nK];
        printf("\nPoint's info : I/O Tag = %I64d, Simple Tag = %I64d, and "
               "details below :",
               GET_POINT_INSIDE(pTempNode->m_n64Type, nIndex),
               GET_POINT_SIMPLE(pTempNode->m_n64SimpleTag, nIndex));
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++)
                for (int k = 0; k < 2; k++)
                    PrintNodeInfo(pNode[i][j][k], n64Position[i][j][k],
                                  nLevel[i][j][k]);
    }
    bool InNode(UCHAR *pNode,
                UINT64 n64Position,
                int nLevel,
                UINT64 n64Search) {
        return ((INT64GETI(n64Search) >> (MAX_LEVEL - nLevel)) ==
                        (INT64GETI(n64Position) >> (MAX_LEVEL - nLevel)) &&
                (INT64GETJ(n64Search) >> (MAX_LEVEL - nLevel)) ==
                        (INT64GETJ(n64Position) >> (MAX_LEVEL - nLevel)) &&
                (INT64GETK(n64Search) >> (MAX_LEVEL - nLevel)) ==
                        (INT64GETK(n64Position) >> (MAX_LEVEL - nLevel)));
    }
    void Search(UCHAR *pNode,
                UINT64 n64Position,
                int nLevel,
                UINT64 n64Search,
                int nSearch,
                UCHAR *&pRtnNode,
                UINT64 &n64RtnPosition,
                int &nRtnLevel) {
        if (!InNode(pNode, n64Position, nLevel, n64Search)) {
            ...
            UINT64 n64Leaf;
            int nLeaf;
            Leaf(pNode, n64Position, nLevel,
                 (int)(INT64GETI(n64Search) >> (MAX_LEVEL - nLevel - 1)) & 1,
                 (int)(INT64GETJ(n64Search) >> (MAX_LEVEL - nLevel - 1)) & 1,
                 (int)(INT64GETK(n64Search) >> (MAX_LEVEL - nLevel - 1)) & 1,
                 pLeafNode, n64Leaf, nLeaf);
            Search(pLeafNode, n64Leaf, nLeaf, n64Search, nSearch, pRtnNode,
                   n64RtnPosition, nRtnLevel);
        }
    }
    void GetFaceFromPos(UINT64 n64Search,
                        int nSearch,
                        int dirFace,
                        UCHAR *pRtnNode[2],
                        UINT64 n64RtnPosition[2],
                        int nRtnLevel[2]) {
        for (int i = 0; i < 2; i++) {
            switch (dirFace) {
                case 0:
                    Search(m_pRoot, 0, 0,
                           INT64TRANS(INT64GETI(n64Search) + i * 2 - 1,
                                      INT64GETJ(n64Search) + 1,
                                      INT64GETK(n64Search) + 1),
                           nSearch, pRtnNode[i], n64RtnPosition[i],
                           nRtnLevel[i]);
                    break;
                case 1:
                    Search(m_pRoot, 0, 0,
                           INT64TRANS(INT64GETI(n64Search) + 1,
                                      INT64GETJ(n64Search) + i * 2 - 1,
                                      INT64GETK(n64Search) + 1),
                           nSearch, pRtnNode[i], n64RtnPosition[i],
                           nRtnLevel[i]);
                    break;
    ...
    void GetEdgeFromPos(UINT64 n64Search,
                        int nSearch,
                        int dirEdge,
                        UCHAR *pRtnNode[2][2],
                        UINT64 n64RtnPosition[2][2],
                        int nRtnLevel[2][2]) {
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++) {
                switch (dirEdge) {
                    case 0:
                        Search(m_pRoot, 0, 0,
                               INT64TRANS(INT64GETI(n64Search) + 1,
                                          INT64GETJ(n64Search) + i * 2 - 1,
                                          INT64GETK(n64Search) + j * 2 - 1),
                               nSearch, pRtnNode[i][j], n64RtnPosition[i][j],
                               nRtnLevel[i][j]);
                        break;
                    case 1:
                        Search(m_pRoot, 0, 0,
                               INT64TRANS(INT64GETI(n64Search) + j * 2 - 1,
                                          INT64GETJ(n64Search) + 1,
                                          INT64GETK(n64Search) + i * 2 - 1),
                               nSearch, pRtnNode[i][j], n64RtnPosition[i][j],
                               nRtnLevel[i][j]);
    ...
    void GetPointFromPos(UINT64 n64Search,
                         int nSearch,
                         UCHAR *pRtnNode[2][2][2],
                         UINT64 n64RtnPosition[2][2][2],
                         int nRtnLevel[2][2][2]) {
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++)
                for (int k = 0; k < 2; k++) {
                    Search(m_pRoot, 0, 0,
                           INT64TRANS(INT64GETI(n64Search) + i * 2 - 1,
                                      INT64GETJ(n64Search) + j * 2 - 1,
                                      INT64GETK(n64Search) + k * 2 - 1),
                           nSearch, pRtnNode[i][j][k], n64RtnPosition[i][j][k],
                           nRtnLevel[i][j][k]);
                }
    }
    void GetFacePos(UINT64 n64Position[2],
                    int nLevel[2],
                    int dirFace,
                    int nI,
                    int iIndex,
                    UINT64 &n64RtnPosition) {
        n64RtnPosition = LeafPosition(
                n64Position[nI], nLevel[nI],
                CConstant::I_SHIFT_FP[CConstant::II_SHIFT_CF[dirFace][nI]]
                                     [iIndex]);
    }
    void GetEdgePos(UINT64 n64Position[2][2],
                    int nLevel[2][2],
                    int dirEdge,
                    int nI,
                    int nJ,
                    ...
                    UINT64 &n64RtnPosition) {
        n64RtnPosition = LeafPosition(
                n64Position[nI][nJ], nLevel[nI][nJ],
                CConstant::I_SHIFT_EP[CConstant::II_SHIFT_CE[dirEdge][nI][nJ]]
                                     [iIndex]);
    }
    void GetPointPos(UINT64 n64Position[2][2][2],
                     int nLevel[2][2][2],
                     int nI,
                     int nJ,
                     int nK,
                     UINT64 &n64RtnPosition) {
        n64RtnPosition =
                LeafPosition(n64Position[nI][nJ][nK], nLevel[nI][nJ][nK],
                             CConstant::II_SHIFT_CP[nI][nJ][nK]);
    }
    UINT64 NodePosition(UINT64 n64Position, int nLevel) {
        return n64Position + INT64TRANS(NLENGTH(nLevel + 1),
                                        NLENGTH(nLevel + 1),
                                        NLENGTH(nLevel + 1));
    }
    UINT64 FacePosition(UINT64 n64Position[2], int nLevel[2], int dirFace) {
        ...
        switch (dirFace) {
            case 0:
                return n64Face + INT64TRANS(0, NLENGTH(nLevel[nI] + 1),
                                            NLENGTH(nLevel[nI] + 1));
                break;
            case 1:
                return n64Face + INT64TRANS(NLENGTH(nLevel[nI] + 1), 0,
                                            NLENGTH(nLevel[nI] + 1));
                break;
            case 2:
                return n64Face + INT64TRANS(NLENGTH(nLevel[nI] + 1),
                                            NLENGTH(nLevel[nI] + 1), 0);
                break;
        }
        return n64Face;
    }
    ...
    UINT64 LeafPosition(UINT64 n64Position, int nLevel, int i, int j, int k) {
        return INT64TRANS(i * NLENGTH(nLevel) + INT64GETI(n64Position),
                          j * NLENGTH(nLevel) + INT64GETJ(n64Position),
                          k * NLENGTH(nLevel) + INT64GETK(n64Position));
    }
    ...
    int LeafLevel(UCHAR *pNode, int nLevel) { return nLevel + 1; }
    void Leaf(UCHAR *pNode,
              UINT64 n64Position,
              int nLevel,
              int i,
              int j,
        ...
        if ((*pNode & 3) == 0) {
            ...
            nNextPos = n64Position + INT64TRANS(i * NLENGTH(nLevel + 1),
                                                j * NLENGTH(nLevel + 1),
                                                k * NLENGTH(nLevel + 1));
            nNextLevel = nLevel + 1;
        } else {
    ...
    void GetStorageEdge(
            int nLevel[2][2], int dirEdge, int &nI, int &nJ, int &nIndex) {
        int nMaxLevel = -1;
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++)
    ...
    void GetStoragePoint(
            int nlevel[2][2][2], int &nI, int &nJ, int &nK, int &nIndex) {
        int nMaxLevel = -1;
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++)
                for (int k = 0; k < 2; k++)
    ...
    void InsertIndex(list<int> &listIndex, int iInsert) {
        for (list<int>::iterator it = listIndex.begin(); it != listIndex.end();
             it++)
            if (iInsert == (*it)) return;
    ...
    void WriteFace(list<int> &listIndex) {
        ...
        int l;
        for (itTemp = listIndex.begin(), l = 0; itTemp != listIndex.end();
             itTemp++, l++)
            pIndex[l] = *itTemp;
            ...
            if (!m_bCountOnly) {
                ...
                switch (m_nFileType) {
                    case 1:  // ply
                        ...
                        reverse_fwrite((UCHAR *)&pIndex[j + 1], 4, 1,
                                       m_pWriteFile);
                        reverse_fwrite((UCHAR *)&pIndex[j + 2], 4, 1,
                                       m_pWriteFile);
                        break;
                    default:  // obj
                        fprintf(m_pWriteFile, "f %d %d %d\n", pIndex[0] + 1,
    ...
    void PrintInfo() {
        ...
        m_nCountPoint = 0;
        Traversal();
        printf("Inside part contains %d cells, %d faces, %d edges, %d "
               "points.\n",
               m_nCountCell, m_nCountFace, m_nCountEdge, m_nCountPoint);
    ...
    void SetScalar(
            double dbScalar) {  // a reversed function, just set a octree
                                // centered at (0, 0, 0), with scalar dbScalar
        m_dbScalar = dbScalar;
        m_dbLength = 2.0 / m_dbScalar;
        m_dbNodeLength = 2.0 / m_dbScalar / (double)NLENGTH(0);
        m_vCorner = CVector3D(-1.0 / m_dbScalar, -1.0 / m_dbScalar,
                              -1.0 / m_dbScalar);
    }
    ...
    void SetObjectInside(UINT64 &n64Type,
                         int iIndex,
                         int nObjType,
                         bool bInside) {
        if (bInside) {
            n64Type |= (1 << (iIndex +
                              CConstant::OBJECT_START_POSITION[nObjType] + 4));
        } else {
            n64Type &=
                    (~(1 << (iIndex +
                             CConstant::OBJECT_START_POSITION[nObjType] + 4)));
    ...
    void SetObjectSimple(UINT64 &n64SimpleTag,
                         int iIndex,
                         int nObjType,
                         UINT64 n64Type) {
        int nShift =
                iIndex * 2 + CConstant::OBJECT_START_POSITION[nObjType] * 2;
    ...
    RETRACE_EDGE *GetRetraceEdge(UCHAR *pNode, int iIndex) {
        ...
        for (int i = 0; i < iIndex; i++)
            if (GET_FACE_RIT(pTempLeaf->m_n64Type, i)) nCount++;
        return ((RETRACE_EDGE *)(pNode + sizeof(LEAF_NODE) +
    ...
    int CheckEdgeNeighborFace(UCHAR *pNode[2][2],
                              int nLevel[2][2],
                              int dirEdge) {
        int nNum = 0;
        UCHAR *pNodeFace[2];
    ...
    bool GetEdgeNeighborPoint(UCHAR *pNode[2][2],
                              UINT64 n64Position[2][2],
                              int nLevel[2][2],
                              int dirEdge,
                              UCHAR *pRtnNode[2][2][2],
        ...
        int nRtn = 0;
        for (int l = 0; l < 2; l++) {
            UINT64 n64PointPosition =
                    LeafPosition(n64Position[nI][nJ], nLevel[nI][nJ],
                                 CConstant::III_SHIFT_CEP[dirEdge][nI][nJ][l]);
            GetPointFromPos(n64PointPosition, MAX_LEVEL, pNodePoint, n64Point,
                            nPoint);
    ...
    float GetEdgeWeight(UCHAR *pNode[2][2],
                        UINT64 n64Position[2][2],
                        int nLevel[2][2],
                        int dirEdge) {
    ...
    int CheckFaceNeighborEdge(UCHAR *pNode[2],
                              UINT64 n64Position[2],
                              int nLevel[2],
                              int dirFace) {
        UINT64 n64Face;
    ...
    int RecCheckFaceNeighborEdge(UCHAR *pNode[2][2],
                                 UINT64 n64Position[2][2],
                                 int nLevel[2][2],
                                 int dirEdge) {
        UCHAR *pNodeEdge[2][2];
        ...
        if (NOT_LEAF(pNode[0][0]) || NOT_LEAF(pNode[0][1]) ||
                        ...
                        for (int j = 0; j < 2; j++)
                            for (int k = 0; k < 2; k++)
                                Leaf(pNode[j][k], n64Position[j][k],
                                     nLevel[i][j], i, 1 - j, 1 - k,
                                     pNodeEdge[j][k], n64Edge[j][k],
                                     nEdge[j][k]);
                        nNum += RecCheckFaceNeighborEdge(pNodeEdge, n64Edge,
                                                         nEdge, dirEdge);
                    }
                    break;
                case 1:
            ...
            if (CheckStorageEdge(pNode)) {
                GetStorageEdge(nLevel, dirEdge, nI, nJ, nIndex);
                if (GET_EDGE_SIMPLE(
                            ((LEAF_NODE *)pNode[nI][nJ])->m_n64SimpleTag,
                            nIndex) == 1)
                    nNum = 1;
            }
        }
    ...
    float GetFaceWeight(UCHAR *pNode[2],
                        UINT64 n64Position[2],
                        int nLevel[2],
                        int dirFace) {
    ...
    bool GetFaceNeighborCell(UCHAR *pNode[2],
                             UINT64 n64Position[2],
                             int nLevel[2],
                             int dirFace,
                             int &nRtnCellIndex) {
        ...
        for (int i = 0; i < 2; i++) {
            if (GET_CELL_SIMPLE(((LEAF_NODE *)pNode[i])->m_n64SimpleTag, 0) ==
                2) {
                nRtn++;
                nRtnCellIndex = i;
            }
...
public:
    ...
    void RecLoadFromSOG(UCHAR **ppNode,
                        FILE *pFile,
                        UINT64 n64Position,
                        int nLevel);
    ...
    void RecTraversalFace(UCHAR *pNode[2],
                          UINT64 n64Position[2],
                          int nLevel[2],
                          int dirFace);
    void RecTraversalEdge(UCHAR *pNode[2][2],
                          UINT64 n64Position[2][2],
                          int nLevel[2][2],
                          int dirEdge);
    void RecTraversalPoint(UCHAR *pNode[2][2][2],
                           UINT64 n64Position[2][2][2],
                           int nLevel[2][2][2]);

    void InitFaceTags();
    void RecInitFaceTagsCell(UCHAR *pNode, UINT64 n64Position, int nLevel);
    void RecInitFaceTagsFace(UCHAR *pNode[2],
                             UINT64 n64Position[2],
                             int nLevel[2],
                             int dirFace);

    void RecRegularTest(UCHAR *pNode, int &nRegularCount);
    void RecCreateFaceFaceEdgeTraversal(UCHAR *pCoNode,
    ...
    bool RecThinFace(UCHAR *pNode[2],
                     UINT64 n64Position[2],
                     int nLevel[2],
                     int dirFace);
    bool RecThinEdge(UCHAR *pNode[2][2],
                     UINT64 n64Position[2][2],
                     int nLevel[2][2],
                     int dirEdge);
    bool RecThinPoint(UCHAR *pNode[2][2][2],
                      UINT64 n64Position[2][2][2],
                      int nLevel[2][2][2]);
    ...
    bool RecDualThinFace(UCHAR *pNode[2],
                         UINT64 n64Position[2],
                         int nLevel[2],
                         int dirFace);
    bool RecDualThinEdge(UCHAR *pNode[2][2],
                         UINT64 n64Position[2][2],
                         int nLevel[2][2],
                         int dirEdge);
    bool RecDualThinPoint(UCHAR *pNode[2][2][2],
                          UINT64 n64Position[2][2][2],
                          int nLevel[2][2][2]);
    void DualCreateMST(float fThreshold, CString strSKTFile);
    void RecPushFace(UCHAR *pNode[2],
                     UINT64 n64Position[2],
                     int nLevel[2],
                     int dirFace,
                     CFaceHeap &heapFace);
    ...
    void RecDualGrowByEdge(UCHAR *pNode[2][2],
                           UINT64 n64Position[2][2],
                           int nLevel[2][2],
                           int dirEdge);
};
