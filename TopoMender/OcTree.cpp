...
COcTree::~COcTree(void) {
    Clear();
    if (m_pHashMap != NULL) delete m_pHashMap;
    if (m_pSingleHashMap != NULL) delete m_pSingleHashMap;
}

void COcTree::Clear() {
    if (m_pMemoryManager != NULL) delete m_pMemoryManager;
}
...
void COcTree::RecLoadFromSOG(UCHAR **ppNode,
                             FILE *pFile,
                             UINT64 n64Position,
                             int nLevel) {
    UCHAR nType;
    ...
    switch (nType) {
                    ...
                    for (int k = 0; k < 2; k++)
                        RecLoadFromSOG(
                                &(((INTER_NODE *)(*ppNode))
                                          ->m_pNode[i * 4 + j * 2 + k]),
                                pFile,
                                INT64TRANS(i * NLENGTH(nLevel) +
                                                   INT64GETI(n64Position),
                                           j * NLENGTH(nLevel) +
                                                   INT64GETJ(n64Position),
                                           k * NLENGTH(nLevel) +
                                                   INT64GETK(n64Position)),
                                nLevel + 1);
            break;
        case 1:
        ...
        case 2:
            ...
            *ppNode = m_pMemoryManager->New(sizeof(LEAF_NODE));
            ...
            if (GET_POINT_INSIDE(n64Temp, 0) && GET_POINT_INSIDE(n64Temp, 1) &&
                GET_POINT_INSIDE(n64Temp, 2) && GET_POINT_INSIDE(n64Temp, 3) &&
                GET_POINT_INSIDE(n64Temp, 4) && GET_POINT_INSIDE(n64Temp, 5) &&
                GET_POINT_INSIDE(n64Temp, 6) && GET_POINT_INSIDE(n64Temp, 7))
                ((LEAF_NODE *)(*ppNode))->m_n64Type |= mask;
            break;
    }
}
        ...
        case 0:
            *ppNode = m_pMemoryManager->New(sizeof(INTER_NODE));
            ...
            for (int i = 0; i < 2; i++)
                for (int j = 0; j < 2; j++)
                    for (int k = 0; k < 2; k++)
                        RecLoadFromSOG(&(((INTER_NODE *)(*ppNode))
                                                 ->m_pNode[i * 4 + j * 2 + k]),
                                       pFile);
            break;
        case 1:
...
void COcTree::RecLoadFromOcTree(UCHAR **ppNode,
                                UCHAR *pOcTreeNode,
                                COcTree *pOctree) {
                ...
                for (int j = 0; j < 2; j++)
                    for (int k = 0; k < 2; k++)
                        RecLoadFromOcTree(
                                &(((INTER_NODE *)(*ppNode))
                                          ->m_pNode[i * 4 + j * 2 + k]),
                                interNodeTemp.m_pNode[i * 4 + j * 2 + k],
                                pOctree);
            break;
        case 2:
        case 1:
            ...
            int nCount = 0;
            for (int i = 0; i < 6; i++)
                if ((leafNodeTemp.m_n64Type >> (56 + i)) & 1) nCount++;
            *ppNode = m_pMemoryManager->New(sizeof(LEAF_NODE) +
                                            nCount * (sizeof(RETRACE_EDGE)));
            m_nCountCell += nCount;
...
void COcTree::RecTraversalCell(UCHAR *pNode, UINT64 n64Position, int nLevel) {
    if (NOT_LEAF(pNode)) {
        ...
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++)
                for (int k = 0; k < 2; k++) {
                    // Leaf(pNode, n64Position, nLevel, i, j, k, pNodeCell,
                    // n64Cell, nCell);
        ...
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++) {
                for (int k = 0; k < 2; k++)
                // Leaf(pNode, n64Position, nLevel, k, i, j, pNodeFace[k],
                // n64Face[k], nFace[k]);
                {
                ...
                for (int k = 0; k < 2; k++)
                // Leaf(pNode, n64Position, nLevel, j, k, i, pNodeFace[k],
                // n64Face[k], nFace[k]);
                {
                ...
                for (int k = 0; k < 2; k++)
                // Leaf(pNode, n64Position, nLevel, i, j, k, pNodeFace[k],
                // n64Face[k], nFace[k]);
                {
        ...
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 2; j++)
                for (int k = 0; k < 2; k++)
                // Leaf(pNode, n64Position, nLevel, i, j, k, pNodeEdge[j][k],
            ...
            for (int j = 0; j < 2; j++)
                for (int k = 0; k < 2; k++)
                // Leaf(pNode, n64Position, nLevel, k, i, j, pNodeEdge[j][k],
                // n64Edge[j][k], nEdge[j][k]);
                {
            ...
            for (int j = 0; j < 2; j++)
                for (int k = 0; k < 2; k++)
                // Leaf(pNode, n64Position, nLevel, j, k, i, pNodeEdge[j][k],
                // n64Edge[j][k], nEdge[j][k]);
                {
...
void COcTree::RecTraversalFace(UCHAR *pNode[2],
        ...
        switch (dirFace) {
            case 0:
                ...
                for (int i = 0; i < 2; i++)
                    for (int j = 0; j < 2; j++) {
                        for (int k = 0; k < 2; k++)
                        // Leaf(pNode[k], n64Position[k], nLevel[k], 1-k, i, j,
                        // pNodeFace[k], n64Face[k], nFace[k]);
                        {
                            pNodeFace[k] = pNodePoint[k][i][j];
                            n64Face[k] = n64Point[k][i][j];
                            nFace[k] = nPoint[k][i][j];
                ...
                for (int i = 0; i < 2; i++) {
                    ...
                    for (int j = 0; j < 2; j++)
                        for (int k = 0; k < 2; k++)
                        // Leaf(pNode[j], n64Position[j], nLevel[j], 1-j, k, i,
                        // pNodeEdge[j][k], n64Edge[j][k], nEdge[j][k]);
                        {
                            pNodeEdge[j][k] = pNodePoint[j][k][i];
                            n64Edge[j][k] = n64Point[j][k][i];
            ...
            case 1:
                ...
                for (int i = 0; i < 2; i++)
                    for (int j = 0; j < 2; j++) {
                        for (int k = 0; k < 2; k++)
                        // Leaf(pNode[k], n64Position[k], nLevel[k], j, 1-k, i,
                        // pNodeFace[k], n64Face[k], nFace[k]);
                        {
                            pNodeFace[k] = pNodePoint[j][k][i];
                            n64Face[k] = n64Point[j][k][i];
                ...
                for (int i = 0; i < 2; i++) {
                    ...
                    for (int j = 0; j < 2; j++)
                        for (int k = 0; k < 2; k++)
                        // Leaf(pNode[j], n64Position[j], nLevel[j], i, 1-j, k,
                        // pNodeEdge[j][k], n64Edge[j][k], nEdge[j][k]);
                        {
                            pNodeEdge[j][k] = pNodePoint[i][j][k];
                            n64Edge[j][k] = n64Point[i][j][k];
            ...
            case 2:
                ...
                for (int i = 0; i < 2; i++)
                    for (int j = 0; j < 2; j++) {
                        for (int k = 0; k < 2; k++)
                        // Leaf(pNode[k], n64Position[k], nLevel[k], i, j, 1-k,
                        // pNodeFace[k], n64Face[k], nFace[k]);
                        {
                            pNodeFace[k] = pNodePoint[i][j][k];
                            n64Face[k] = n64Point[i][j][k];
                ...
                for (int i = 0; i < 2; i++) {
                    ...
                    for (int j = 0; j < 2; j++)
                        for (int k = 0; k < 2; k++)
                        // Leaf(pNode[j], n64Position[j], nLevel[j], k, i, 1-j,
                        // pNodeEdge[j][k], n64Edge[j][k], nEdge[j][k]);
                        {
                            pNodeEdge[j][k] = pNodePoint[k][i][j];
                            n64Edge[j][k] = n64Point[k][i][j];
        ...
        switch (m_nType) {
            case -1:
                if (GET_FACE_INSIDE(pTempNode->m_n64Type, nIndex) == 0) {
                    for (int i = 0; i < 2; i++) {
                        if (GET_CELL_INSIDE(((LEAF_NODE *)pNode[i])->m_n64Type,
                                            0) != 0) {  // error
                ...
                if (GET_FACE_INSIDE(pTempNode->m_n64Type, nIndex)) {
                    for (int i = 0; i < 2; i++)
                        if (GET_CELL_INSIDE(((LEAF_NODE *)pNode[i])->m_n64Type,
                            ...
                            GetFacePos(n64Position, nLevel, dirFace, nI, 0,
                                       n64Face);
                            dirEdge = (dirFace + 2 - i) % 3;
                            GetEdgeFromPos(n64Face, nLevel[nI], dirEdge,
                                           pNodeEdge, n64Edge, nEdge);
                            RecCreateFaceFaceEdgeTraversal(
                                    pNode[i], n64Position[i], nLevel[i],
                                    pNodeEdge, n64Edge, nEdge, dirEdge, 0,
                                    listIndex);

                            GetFacePos(n64Position, nLevel, dirFace, nI, 1 + i,
                                       n64Face);
                            ...
                            GetFacePos(n64Position, nLevel, dirFace, nI, 2 - i,
                                       n64Face);
                            dirEdge = (dirFace + 2 - i) % 3;
                            GetEdgeFromPos(n64Face, nLevel[nI], dirEdge,
                                           pNodeEdge, n64Edge, nEdge);
                            RecCreateFaceFaceEdgeTraversal(
                                    pNode[i], n64Position[i], nLevel[i],
                                    pNodeEdge, n64Edge, nEdge, dirEdge, 1,
                                    listIndex);
            ...
            case 5:
                if (GET_FACE_SIMPLE(pTempNode->m_n64SimpleTag, nIndex) == 3) {
                    for (int i = 0; i < 2; i++) {
                        if (nLevel[i] == nLevel[nI]) {
                            SetObjectSimple(
                                    ((LEAF_NODE *)pNode[i])->m_n64SimpleTag,
                                    dirFace * 2 + (1 - i), 2, 3);
                        }
                    }
                }
                ...
                //{
                //	if (nLevel[i] < nLevel[nI]) {
                //		SetObjectInside(((LEAF_NODE
                //*)pNode[i])->m_n64Type, dirFace * 2 + (1 - i), 2, false); 	}
                //else { 		SetObjectInside(((LEAF_NODE *)pNode[i])->m_n64Type,
                //dirFace * 2 + (1 - i), 2,
                //GET_FACE_INSIDE(pTempNode->m_n64Type, nIndex));
                //	}
                //}
                break;
            case 7:
...
void COcTree::RecTraversalEdge(UCHAR *pNode[2][2],
        ...
        switch (dirEdge) {
            case 0:
                ...
                for (int i = 0; i < 2; i++) {
                    for (int j = 0; j < 2; j++)
                        for (int k = 0; k < 2; k++)
                        // Leaf(pNode[j][k], n64Position[j][k], nLevel[j][k], i,
                        // 1-j, 1-k, pNodeEdge[j][k], n64Edge[j][k],
                        // nEdge[j][k]);
                        {
                            pNodeEdge[j][k] = pNodePoint[i][j][k];
                            n64Edge[j][k] = n64Point[i][j][k];
            ...
            case 1:
                ...
                for (int i = 0; i < 2; i++) {
                    for (int j = 0; j < 2; j++)
                        for (int k = 0; k < 2; k++)
                        // Leaf(pNode[j][k], n64Position[j][k], nLevel[j][k],
                        // 1-k, i, 1-j, pNodeEdge[j][k], n64Edge[j][k],
                        // nEdge[j][k]);
                        {
                            pNodeEdge[j][k] = pNodePoint[k][i][j];
                            n64Edge[j][k] = n64Point[k][i][j];
            ...
            case 2:
                ...
                for (int i = 0; i < 2; i++) {
                    for (int j = 0; j < 2; j++)
                        for (int k = 0; k < 2; k++)
                        // Leaf(pNode[j][k], n64Position[j][k], nLevel[j][k],
                        // 1-j, 1-k, i, pNodeEdge[j][k], n64Edge[j][k],
                        // nEdge[j][k]);
                        {
                            pNodeEdge[j][k] = pNodePoint[j][k][i];
                            n64Edge[j][k] = n64Point[j][k][i];
        ...
        switch (m_nType) {
                ...
                if (GET_EDGE_INSIDE(pTempNode->m_n64Type, nIndex) == 0) {
                    ...
                    for (int i = 0; i < 2; i++) {
                        ...
                        if (CheckStorageFace(pNodeFace)) {
                            ...
                            if (GET_FACE_INSIDE(((LEAF_NODE *)pNodeFace[nFaceI])
                                                        ->m_n64Type,
                                                nFaceIndex) != 0) {  // error
            ...
            case 4:
                if (GET_EDGE_INSIDE(pTempNode->m_n64Type, nIndex)) {
                    ...
                    list<int> listIndex;
                    UINT64 n64Edge[2];
                    GetEdgePos(n64Position, nLevel, dirEdge, nI, nJ, 0,
                               n64Edge[0]);
                    GetEdgePos(n64Position, nLevel, dirEdge, nI, nJ, 1,
                               n64Edge[1]);
                    for (int i = 0; i < 2; i++) {
                        listIndex.clear();
                        for (int j = 0; j < 2; j++) {
                        ...
                        if (!GetFaceInside(pNodeFace, nFace, dirFace)) {
                            InsertIndex(listIndex,
                                        GetIndex(pNodeFace[i], n64Face[i],
                                                 n64Edge[0]));
                            InsertIndex(listIndex,
                                        GetIndex(pNodeFace[i], n64Face[i],
                                                 n64Edge[1]));
                            InsertIndex(listIndex,
                                        GetIndex(pNodeFace[1 - i],
                                                 n64Face[1 - i], n64Edge[1]));
                            InsertIndex(listIndex,
                                        GetIndex(pNodeFace[1 - i],
                                                 n64Face[1 - i], n64Edge[0]));
                            if (listIndex.size() >= 3) {
                                WriteFace(listIndex, m_nFaceNum);
                                // m_nFaceNum ++;
                            }
            ...
            case 5:
                ...
                //		if (nLevel[i][j] < nLevel[nI][nJ]) {
                //			SetObjectInside(((LEAF_NODE
                //*)pNode[i][j])->m_n64Type, dirEdge * 4 + (1 - i) * 2 + (1 -
                //j), 1, false); 		} else { 			SetObjectInside(((LEAF_NODE
                //*)pNode[i][j])->m_n64Type, dirEdge * 4 + (1 - i) * 2 + (1 -
                //j), 1, GET_EDGE_INSIDE(pTempNode->m_n64Type, nIndex));
                //		}
                //	}
                break;
            case 6:
...
void COcTree::RecTraversalPoint(UCHAR *pNode[2][2][2],
        ...
        switch (m_nType) {
                ...
                if (GET_POINT_INSIDE(pTempNode->m_n64Type, nIndex) == 0) {
                    ...
                    for (int i = 0; i < 2; i++) {
                        ...
                        if (CheckStorageEdge(pNodeEdge)) {
                            dirEdge = 0;
                            GetStorageEdge(nEdge, dirEdge, nEdgeI, nEdgeJ,
                                           nEdgeIndex);
                            if (GET_EDGE_INSIDE(
                                        ((LEAF_NODE *)pNodeEdge[nEdgeI][nEdgeJ])
                                                ->m_n64Type,
                                        nEdgeIndex) != 0) {  // error
            ...
            case 2:
                for (int i = 0; i < 2; i++)
                    for (int j = 0; j < 2; j++)
                        for (int k = 0; k < 2; k++)
                            if (GET_POINT_INSIDE(pTempNode->m_n64Type,
                                                 nIndex) &&
                                GET_CELL_INSIDE(((LEAF_NODE *)pNode[i][j][k])
                                                        ->m_n64Type,
                                                0) == 0 &&
                                ((LEAF_NODE *)pNode[i][j][k])->m_nVertex ==
                                        0xffffffff) {
                        ...
                        for (int k = 0; k < 2; k++)
                            if (GET_POINT_INSIDE(pTempNode->m_n64Type,
                                ...
                                ((LEAF_NODE *)pNode[i][j][k])->m_nVertex ==
                                        0xffffffff) {
                                UINT64 n64TempPoint =
                                        n64Position[nI][nJ][nK] +
                                        INT64TRANS(
                                                (1 - nI) *
                                                        NLENGTH(nLevel[nI][nJ]
                                                                      [nK]),
                                                (1 - nJ) *
                                                        NLENGTH(nLevel[nI][nJ]
                                                                      [nK]),
                                                (1 - nK) *
                                                        NLENGTH(nLevel[nI][nJ]
                                                                      [nK]));
                                LEAF_NODE *pLeafNode =
                                        (LEAF_NODE *)pNode[i][j][k];
                                if (m_bCountOnly) {
                                    if (m_pHashMap->Get(n64Position[i][j][k],
                                                        n64TempPoint) == -1) {
                                        m_pHashMap->Set(n64Position[i][j][k],
                                                        n64TempPoint,
                                                        m_nVertexNum);
                                        m_nIrregularCount++;
                                        m_nVertexNum++;
                                    ...
                                    if (bWrite) {
                                        ...
                                        // if
                                        // (!GET_CHANGED(pLeafNode->m_n64Type) &&
                                        // (*pNode[i][j][k] & 0x3) == 2) {
                                        if ((*pNode[i][j][k] & 0x3) == 2) {
                                            CVector3D vCell =
                                                    m_pSingleHashMap->Get(
                                                            (int)pLeafNode);
                                            if (vCell.pVec[0] != 1e300 &&
                                                !GET_CHANGED(
                                                        pLeafNode->m_n64Type)) {
                                                fPos[0] =
                                                        (float)(0.05 * (m_dbNodeLength *
                                                                                (INT64GETI(
                                                                                        n64TempPoint)) +
                                                                        m_vCorner
                                                                                .pVec[0]) +
                ...
                if (GET_POINT_INSIDE(pTempNode->m_n64Type, nIndex)) {
                    ...
                    for (int i = 0; i < 2; i++) {
                        ...
                        if (!GetEdgeInside(pNodeEdge, nEdge, dirEdge)) {
                            InsertIndex(listIndex,
                                        GetIndex(pNodeEdge[0][0], n64Edge[0][0],
                                                 n64Point));
                            InsertIndex(listIndex,
                                        GetIndex(pNodeEdge[i][1 - i],
                                                 n64Edge[i][1 - i], n64Point));
                            InsertIndex(listIndex,
                                        GetIndex(pNodeEdge[1][1], n64Edge[1][1],
                                                 n64Point));
                            InsertIndex(listIndex,
                                        GetIndex(pNodeEdge[1 - i][i],
                                                 n64Edge[1 - i][i], n64Point));
                            if (listIndex.size() >= 3) {
                                WriteFace(listIndex, m_nFaceNum);
                                // m_nFaceNum ++;
                            }
            ...
            case 5:
                if (GET_POINT_SIMPLE(pTempNode->m_n64SimpleTag, nIndex) == 3) {
                    for (int i = 0; i < 2; i++)
                        for (int j = 0; j < 2; j++)
                            for (int k = 0; k < 2; k++)
                                if (pNode[i][j][k] != pNode[1 - i][j][k] &&
                                    pNode[i][j][k] != pNode[i][1 - j][k] &&
                                    pNode[i][j][k] != pNode[i][j][1 - k])
                                    SetObjectSimple(
                                            ((LEAF_NODE *)pNode[i][j][k])
                                                    ->m_n64SimpleTag,
                                            (1 - i) * 4 + (1 - j) * 2 + (1 - k),
                ...
                //		for (int k = 0; k < 2; k++)
                //			if (pNode[i][j][k] != pNode[1-i][j][k] &&
                //pNode[i][j][k] != pNode[i][1-j][k] && pNode[i][j][k] !=
                //pNode[i][j][1-k]) 				SetObjectInside(((LEAF_NODE
                //*)pNode[i][j][k])->m_n64Type, (1 - i) * 4 + (1 - j) * 2 + (1 -
                //k), 0, GET_POINT_INSIDE(pTempNode->m_n64Type, nIndex));
                break;
            case 6:
...
void COcTree::RecInitFaceTagsCell(UCHAR *pNode,
                                  UINT64 n64Position,
                                  int nLevel) {
    UCHAR *pNodeCell;
    UCHAR *pNodeFace[2];
...
void COcTree::RecInitFaceTagsFace(UCHAR *pNode[2],
                                  UINT64 n64Position[2],
                                  int nLevel[2],
                                  int dirFace) {
    UCHAR *pNodeFace[2];
...
void COcTree::ContouringQD(CString strFile, CString strSOGFile) {
    ...
    m_pSingleHashMap =
            new CSingleHashMap(m_pConstant, m_nIrregularUnchangedCell);
...
void COcTree::RecRegularTest(UCHAR *pNode, int &nRegularCount) {
    ...
    UCHAR nNext;
    float fNext[3];
    if (!m_bCountOnly) fread(&nType, 1, 1, m_pSOGFile);
    ...
    if (NOT_LEAF(pNode)) {
        ...
        if (!m_bCountOnly) fread(&nNext, 1, 1, m_pSOGFile);
        if (!m_bCountOnly && (*pNode & 0x3) == 2)
            fread(fNext, 4, 3, m_pSOGFile);
        LEAF_NODE *pTempNode = (LEAF_NODE *)pNode;
        if ((*pNode & 0x3) == 2 &&
            GET_CELL_INSIDE(pTempNode->m_n64Type, 0) == 0) {
            int nPointTag = (int)(pTempNode->m_n64Type >> 4) & 0xff;

            // if (!GET_CHANGED(pTempNode->m_n64Type)) {
            bool bRegularTest =
                    (m_pConstant->m_pRegularTableEx[nPointTag][2] == 1);
            if (bRegularTest) {
                for (int i = 0; i < 12; i++)
                    if (!GET_EDGE_INSIDE(pTempNode->m_n64Type, i) &&
                        GET_POINT_INSIDE(pTempNode->m_n64Type,
                                         CConstant::I_SHIFT_EP[i][0]) &&
                        GET_POINT_INSIDE(pTempNode->m_n64Type,
                                         CConstant::I_SHIFT_EP[i][1]))
                        bRegularTest = false;
            }
            if (bRegularTest) {
                for (int i = 0; i < 6; i++)
                    if (!GET_FACE_INSIDE(pTempNode->m_n64Type, i) &&
                        GET_POINT_INSIDE(pTempNode->m_n64Type,
                                         CConstant::I_SHIFT_FP[i][0]) &&
                        GET_POINT_INSIDE(pTempNode->m_n64Type,
                                         CConstant::I_SHIFT_FP[i][1]) &&
                        GET_POINT_INSIDE(pTempNode->m_n64Type,
                                         CConstant::I_SHIFT_FP[i][2]) &&
                        GET_POINT_INSIDE(pTempNode->m_n64Type,
                                         CConstant::I_SHIFT_FP[i][3]))
                        bRegularTest = false;
            }
...
void COcTree::RecCreateFaceFaceEdgeTraversal(UCHAR *pCoNode,
                    ...
                    for (int j = 0; j < 2; j++)
                        for (int k = 0; k < 2; k++)
                            Leaf(pNode[j][k], n64Position[j][k], nLevel[j][k],
                                 ...
                                 pNodeEdge[j][k], n64Edge[j][k], nEdge[j][k]);
                    RecCreateFaceFaceEdgeTraversal(
                            pCoNode, n64CoPosition, nCoLevel, pNodeEdge,
                            n64Edge, nEdge, dirEdge, direction, listIndex);
                }
                break;
            case 1:
                for (int i = 0; i < 2; i++) {
...
void COcTree::Thin() {
    ...
    printf("\n\tMain thinning process finished.\n\tTime consuming %d.%d "
           "second(s) in %d step(s).",
           (dwEnd - dwStart) / 1000, (dwEnd - dwStart) % 1000, m_nStep);
    ...
    if (NOT_LEAF(pNode)) {
        ...
        switch (nType) {
            case 0:
                for (int i = 0; i < 8; i++)
                    SetObjectSimple(
                            pTempNode->m_n64SimpleTag, i, 0,
                            GET_POINT_INSIDE(pTempNode->m_n64Type, i) ? 0 : 1);
                for (int i = 0; i < 12; i++)
                    SetObjectSimple(
                            pTempNode->m_n64SimpleTag, i, 1,
                            GET_EDGE_INSIDE(pTempNode->m_n64Type, i) ? 0 : 1);
                for (int i = 0; i < 6; i++)
                    SetObjectSimple(
                            pTempNode->m_n64SimpleTag, i, 2,
                            GET_FACE_INSIDE(pTempNode->m_n64Type, i) ? 0 : 1);
                for (int i = 0; i < 1; i++)
                    SetObjectSimple(
                            pTempNode->m_n64SimpleTag, i, 3,
                            GET_CELL_INSIDE(pTempNode->m_n64Type, i) ? 0 : 1);
                for (int i = 0; i < 6; i++)
                    if (GET_FACE_RIT(pTempNode->m_n64Type, i)) {
                        ((RETRACE_EDGE *)pTemp)->m_fWeight = 0.0;
                        ((RETRACE_EDGE *)pTemp)->m_n64PositionEdge =
                                INT64_INVALID;
            ...
            case 1:
                for (int i = 0; i < 8; i++)
                    SetObjectInside(pTempNode->m_n64Type, i, 0,
                                    GET_POINT_SIMPLE(pTempNode->m_n64SimpleTag,
                                                     i) == 0);
                for (int i = 0; i < 12; i++)
                    SetObjectInside(
                            pTempNode->m_n64Type, i, 1,
                            GET_EDGE_SIMPLE(pTempNode->m_n64SimpleTag, i) == 0);
                for (int i = 0; i < 6; i++)
                    SetObjectInside(
                            pTempNode->m_n64Type, i, 2,
                            GET_FACE_SIMPLE(pTempNode->m_n64SimpleTag, i) == 0);
                for (int i = 0; i < 1; i++)
                    SetObjectInside(
                            pTempNode->m_n64Type, i, 3,
                            GET_CELL_SIMPLE(pTempNode->m_n64SimpleTag, i) == 0);
                break;
            case 2:
                n64Temp = pTempNode->m_n64Type;
                for (int i = 0; i < 8; i++)
                    SetObjectInside(
                            pTempNode->m_n64Type, i, 0,
                            GET_POINT_SIMPLE(pTempNode->m_n64SimpleTag, i) !=
                                            3 &&
                                    GET_POINT_INSIDE(pTempNode->m_n64Type, i));
                for (int i = 0; i < 12; i++)
                    SetObjectInside(
                            pTempNode->m_n64Type, i, 1,
                            GET_EDGE_SIMPLE(pTempNode->m_n64SimpleTag, i) !=
                                            3 &&
                                    GET_EDGE_INSIDE(pTempNode->m_n64Type, i));
                for (int i = 0; i < 6; i++)
                    SetObjectInside(
                            pTempNode->m_n64Type, i, 2,
                            GET_FACE_SIMPLE(pTempNode->m_n64SimpleTag, i) !=
                                            3 &&
                                    GET_FACE_INSIDE(pTempNode->m_n64Type, i));
                for (int i = 0; i < 1; i++)
                    SetObjectInside(
                            pTempNode->m_n64Type, i, 3,
                            GET_CELL_SIMPLE(pTempNode->m_n64SimpleTag, i) !=
                                            3 &&
                                    GET_CELL_INSIDE(pTempNode->m_n64Type, i));
...
bool COcTree::RecThinCell(UCHAR *pNode, UINT64 n64Position, int nLevel) {
    ...
    if (!GET_LAST_TIME(pNode)) return false;
        ...
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++)
                for (int k = 0; k < 2; k++) {
                    ...
                    n64Cell = n64Point[i][j][k];
                    nCell = nPoint[i][j][k];
                    if (RecThinCell(pNodeCell, n64Cell, nCell)) bRtn = true;
                }
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++) {
                for (int k = 0; k < 2; k++) {
                ...
                if (RecThinFace(pNodeFace, n64Face, nFace, 0)) bRtn = true;
                for (int k = 0; k < 2; k++) {
                    pNodeFace[k] = pNodePoint[j][k][i];
                    n64Face[k] = n64Point[j][k][i];
                    nFace[k] = nPoint[j][k][i];
                }
                if (RecThinFace(pNodeFace, n64Face, nFace, 1)) bRtn = true;
                for (int k = 0; k < 2; k++) {
                    pNodeFace[k] = pNodePoint[i][j][k];
                    n64Face[k] = n64Point[i][j][k];
                    nFace[k] = nPoint[i][j][k];
                }
                if (RecThinFace(pNodeFace, n64Face, nFace, 2)) bRtn = true;
            }
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 2; j++)
                for (int k = 0; k < 2; k++) {
            ...
            if (RecThinEdge(pNodeEdge, n64Edge, nEdge, 0)) bRtn = true;
            for (int j = 0; j < 2; j++)
                for (int k = 0; k < 2; k++) {
                    pNodeEdge[j][k] = pNodePoint[k][i][j];
                    n64Edge[j][k] = n64Point[k][i][j];
            ...
            if (RecThinEdge(pNodeEdge, n64Edge, nEdge, 1)) bRtn = true;
            for (int j = 0; j < 2; j++)
                for (int k = 0; k < 2; k++) {
                    pNodeEdge[j][k] = pNodePoint[j][k][i];
                    n64Edge[j][k] = n64Point[j][k][i];
            ...
            if (RecThinEdge(pNodeEdge, n64Edge, nEdge, 2)) bRtn = true;
        }
        if (RecThinPoint(pNodePoint, n64Point, nPoint)) bRtn = true;
    } else {
        LEAF_NODE *pTempNode = (LEAF_NODE *)pNode;
...
bool COcTree::RecThinFace(UCHAR *pNode[2],
                          UINT64 n64Position[2],
                          int nLevel[2],
                          int dirFace) {
    if (!GET_LAST_FIX(pNode[0]) && !GET_LAST_FIX(pNode[1])) return false;
    ...
    if ((*(pNode[0]) & 0x3) == 0 || (*(pNode[1]) & 0x3) == 0) {
        ...
        if (nSimpleTag == m_nTestLastOut) {
            ...
            for (int i = 0; i < 2; i++) {
                nSimpleTag = GET_CELL_SIMPLE(
                        ((LEAF_NODE *)pNode[i])->m_n64SimpleTag, 0);
            ...
            if (nSimpleTest == 1) {
                SetObjectSimple(pTempNode->m_n64SimpleTag, nIndex, 2,
                                m_nTestOut);
                SetObjectSimple(((LEAF_NODE *)pNodeUniqueCell)->m_n64SimpleTag,
                                0, 3, m_nTestOut);
                bRtn = true;
            }
        }
    ...
    if (bRtn)
        for (int i = 0; i < 2; i++) SetLastTime(pNode[i], bRtn);

    return bRtn;
}

bool COcTree::RecThinEdge(UCHAR *pNode[2][2],
                          UINT64 n64Position[2][2],
                          int nLevel[2][2],
                          int dirEdge) {
    if (!GET_LAST_FIX(pNode[0][0]) && !GET_LAST_FIX(pNode[0][1]) &&
        !GET_LAST_FIX(pNode[1][0]) && !GET_LAST_FIX(pNode[1][1]))
        return false;
        ...
        if (nSimpleTag == m_nTestLastOut) {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if (CheckStorageFace(pNodeFace)) {
                    GetStorageFace(nFace, dirFace, nFaceI, nFaceIndex);
                    pRetraceEdge =
                            GetRetraceEdge(pNodeFace[nFaceI], nFaceIndex);
                    fWeight += pRetraceEdge->m_fWeight;
                    nSimpleTag = GET_FACE_SIMPLE(
                            ((LEAF_NODE *)pNodeFace[nFaceI])->m_n64SimpleTag,
                            nFaceIndex);
                    if (nSimpleTag == m_nTestOut) {
                        nSimpleTest = 100;
                        break;
            ...
            if (nSimpleTest == 1) {
                ...
                SetObjectSimple(pTempNode->m_n64SimpleTag, nIndex, 1,
                                m_nTestOut);
                UINT64 n64Edge;
                GetEdgePos(n64Position, nLevel, dirEdge, nI, nJ, 0, n64Edge);
                SetObjectSimple(((LEAF_NODE *)pNodeUniqueFace)->m_n64SimpleTag,
                                nUniqueFaceIndex, 2, m_nTestOut);
                pRetraceEdge =
                        GetRetraceEdge(pNodeUniqueFace, nUniqueFaceIndex);
    ...
    if (bRtn)
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++) SetLastTime(pNode[i][j], bRtn);

    return bRtn;
}

bool COcTree::RecThinPoint(UCHAR *pNode[2][2][2],
                           UINT64 n64Position[2][2][2],
                           int nLevel[2][2][2]) {
    if (!GET_LAST_FIX(pNode[0][0][0]) && !GET_LAST_FIX(pNode[0][0][1]) &&
        !GET_LAST_FIX(pNode[0][1][0]) && !GET_LAST_FIX(pNode[0][1][1]) &&
        !GET_LAST_FIX(pNode[1][0][0]) && !GET_LAST_FIX(pNode[1][0][1]) &&
        !GET_LAST_FIX(pNode[1][1][0]) && !GET_LAST_FIX(pNode[1][1][1]))
        return false;
    ...
    if ((*(pNode[0][0][0]) & 0x3) == 0 || (*(pNode[0][0][1]) & 0x3) == 0 ||
        ...
        if (nSimpleTag == m_nTestLastOut) {
            ...
            if (nSimpleTest == 1) {
                // SetObjectSimple(pTempNode->m_n64SimpleTag, nIndex, 0,
                // m_nTestOut); SetObjectSimple(((LEAF_NODE
                // *)pNodeUniqueEdge)->m_n64SimpleTag, nUniqueEdgeIndex, 1,
                // m_nTestOut);
                SetObjectSimple(pTempNode->m_n64SimpleTag, nIndex, 0,
                                m_nTestOut);
                SetObjectSimple(((LEAF_NODE *)pNodeUniqueEdge)->m_n64SimpleTag,
                                nUniqueEdgeIndex, 1, m_nTestOut);
                bRtn = true;
            }
        }
    }
    ...
    if (bRtn)
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++)
                for (int k = 0; k < 2; k++) SetLastTime(pNode[i][j][k], bRtn);
...
void COcTree::CreateMST(float fThreshold, CString strSKTFile) {
    ...
    SetObjectSimple(((LEAF_NODE *)m_pPointStart[nI][nJ][nK])->m_n64SimpleTag,
                    nIndex, 0, 0);
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (CheckStorageEdge(pNodeEdge)) {
            GetStorageEdge(nEdge, dirEdge, nEdgeI, nEdgeJ, nEdgeIndex);
            if (GET_EDGE_SIMPLE(((LEAF_NODE *)pNodeEdge[nEdgeI][nEdgeJ])
                                        ->m_n64SimpleTag,
                                nEdgeIndex) == 2) {
                SetObjectSimple(((LEAF_NODE *)pNodeEdge[nEdgeI][nEdgeJ])
                                        ->m_n64SimpleTag,
                                nEdgeIndex, 1, 3);
    ...
    while (!heapEdge.Empty()) {
        ...
        if (GetEdgeNeighborPoint(cElement.m_pNode, cElement.m_n64Position,
                                 cElement.m_nLevel, cElement.m_dirEdge,
                                 pNodePoint, n64Point, nPoint) ||
            cElement.m_fValue > fThreshold) {
            SetObjectSimple(
                    ((LEAF_NODE *)cElement.m_pNode[nI][nJ])->m_n64SimpleTag,
                    nIndex, 1, 0);
            GetStoragePoint(nPoint, nI, nJ, nK, nIndex);
            SetObjectSimple(
                    ((LEAF_NODE *)pNodePoint[nI][nJ][nK])->m_n64SimpleTag,
                    nIndex, 0, 0);
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 2; j++)
                    for (int k = 0; k < 2; k++) {
                ...
                if (CheckStorageEdge(pNodeEdge)) {
                    ...
                    if (GET_EDGE_SIMPLE(((LEAF_NODE *)pNodeEdge[nEdgeI][nEdgeJ])
                        ...
                        SetObjectSimple(((LEAF_NODE *)pNodeEdge[nEdgeI][nEdgeJ])
                                                ->m_n64SimpleTag,
                                        nEdgeIndex, 1, 3);
                        fEdge = GetEdgeWeight(pNodeEdge, n64Edge, nEdge,
                                              dirEdge);
                        heapEdge.Push(pNodeEdge, n64Edge, nEdge, dirEdge,
                                      fEdge);
                    }
                }
                for (int j = 0; j < 2; j++)
            ...
            if (bOutput) {
                ...
                GetStorageEdge(cElement.m_nLevel, cElement.m_dirEdge, nI, nJ,
                               nIndex);
                GetEdgePos(cElement.m_n64Position, cElement.m_nLevel,
                           cElement.m_dirEdge, nI, nJ, 0, n64Pos);
                fwrite(&n64Pos, sizeof(UINT64), 1, pFile);
                GetEdgePos(cElement.m_n64Position, cElement.m_nLevel,
                           cElement.m_dirEdge, nI, nJ, 1, n64Pos);
                fwrite(&n64Pos, sizeof(UINT64), 1, pFile);
            }
        } else {
            printf("\n\t\tA cut with weight : %8.6f", cElement.m_fValue);
            // SetObjectSimple(((LEAF_NODE
            // *)cElement.m_pNode[nI][nJ])->m_n64SimpleTag, nIndex, 1, 1);
...
void COcTree::Grow() {
    ...
    for (it = m_listGrowSeedEdge.begin(); it != m_listGrowSeedEdge.end();
         it++) {
        GrowByEdge(*it);
    }
...
void COcTree::GrowByEdge(CEdgeHeapElement &cElement) {
    ...
    for (int i = 0; i < 2; i++)
        for (int j = 0; j < 2; j++) {
            if (GET_CELL_INSIDE(
                        ((LEAF_NODE *)cElement.m_pNode[i][j])->m_n64Type, 0)) {
                SetObjectSimple(
                        ((LEAF_NODE *)cElement.m_pNode[i][j])->m_n64SimpleTag,
                        0, 3, 3);
            }
        }
    for (int i = 0; i < 2; i++) {
        ...
        if (CheckStorageFace(pNodeFace)) {
            ...
            if (GET_FACE_SIMPLE(
                        ((LEAF_NODE *)pNodeFace[nFaceI])->m_n64SimpleTag,
                        nFaceIndex) != 3 &&
                GET_FACE_INSIDE(((LEAF_NODE *)pNodeFace[nFaceI])->m_n64Type,
                                nFaceIndex)) {
                SetObjectSimple(
                        ((LEAF_NODE *)pNodeFace[nFaceI])->m_n64SimpleTag,
                        nFaceIndex, 2, 3);
                ...
                if (pRetraceEdge->m_n64PositionEdge != INT64_INVALID) {
                    ...
                    GetEdgeFromPos(INT64GETN(pRetraceEdge->m_n64PositionEdge),
                                   MAX_LEVEL,
                    ...
                    if (GET_EDGE_SIMPLE(((LEAF_NODE *)pNodeEdge[nEdgeI][nEdgeJ])
                        ...
                        SetObjectSimple(((LEAF_NODE *)pNodeEdge[nEdgeI][nEdgeJ])
                                                ->m_n64SimpleTag,
                                        nEdgeIndex, 1, 3);
                        GrowByEdge(CEdgeHeapElement(pNodeEdge, n64Edge, nEdge,
                                                    dirEdge, 0.0f));
                    }
                }
            }
    ...
    for (int i = 0; i < 2; i++)
        for (int j = 0; j < 2; j++)
            for (int k = 0; k < 2; k++) {
                ...
                n64Point[i][j][k] = 0;
                nPoint[i][j][k] = 0;
                if (RecDualThinPoint(pNodePoint, n64Point, nPoint)) bRtn = true;
            }
    for (int i = 0; i < 2; i++)
        for (int j = 0; j < 2; j++) {
            for (int ii = 0; ii < 2; ii++)
            ...
            n64Edge[i][j] = 0;
            nEdge[i][j] = 0;
            if (RecDualThinEdge(pNodeEdge, n64Edge, nEdge, 0)) bRtn = true;
            if (RecDualThinEdge(pNodeEdge, n64Edge, nEdge, 1)) bRtn = true;
            if (RecDualThinEdge(pNodeEdge, n64Edge, nEdge, 2)) bRtn = true;
        }
    for (int i = 0; i < 2; i++) {
        for (int ii = 0; ii < 2; ii++) {
            pNodeFace[ii] = (UCHAR *)(&m_cInfiniteNode);
        ...
        n64Face[i] = 0;
        nFace[i] = 0;
        if (RecDualThinFace(pNodeFace, n64Face, nFace, 0)) bRtn = true;
        if (RecDualThinFace(pNodeFace, n64Face, nFace, 1)) bRtn = true;
        if (RecDualThinFace(pNodeFace, n64Face, nFace, 2)) bRtn = true;
    }
    if (RecDualThinCell(m_pRoot, 0, 0)) bRtn = true;
    return bRtn;
}
...
bool COcTree::RecDualThinCell(UCHAR *pNode, UINT64 n64Position, int nLevel) {
    ...
    if (!m_bCountOnly) {
        if (GET_LAST_TIME(pNode)) bRtn = true;
...
bool COcTree::RecDualThinFace(UCHAR *pNode[2],
                              UINT64 n64Position[2],
                              int nLevel[2],
                              int dirFace) {
    bool bRtn = false;
    ...
    if ((*(pNode[0]) & 0x3) == 0 || (*(pNode[1]) & 0x3) == 0) {
            ...
            switch (dirFace) {
                case 0:
                    for (int i = 0; i < 2; i++)
                        for (int j = 0; j < 2; j++)
                            for (int k = 0; k < 2; k++)
                                Leaf(pNode[i], n64Position[i], nLevel[i], 1 - i,
                                     j, k, pNodePoint[i][j][k],
                                     n64Point[i][j][k], nPoint[i][j][k]);
                ...
                case 1:
                    for (int i = 0; i < 2; i++)
                        for (int j = 0; j < 2; j++)
                            for (int k = 0; k < 2; k++)
                                Leaf(pNode[j], n64Position[j], nLevel[j], i,
                                     1 - j, k, pNodePoint[i][j][k],
                                     n64Point[i][j][k], nPoint[i][j][k]);
                    if (RecDualThinPoint(pNodePoint, n64Point, nPoint))
                        bRtn = true;
                    for (int i = 0; i < 2; i++) {
        ...
        if (nSimpleTag != 0) {
            if (m_bCountOnly) {
                ...
                for (int i = 0; i < 2; i++)
                    if (GET_CELL_SIMPLE(((LEAF_NODE *)pNode[i])->m_n64SimpleTag,
                                        0) == 2)
                        break;
                if (i < 2) {
                    SetObjectSimple(pTempNode->m_n64SimpleTag, nIndex, 2, 0);
                    SetObjectSimple(((LEAF_NODE *)pNode[i])->m_n64SimpleTag, 0,
                                    3, 0);
#ifdef CLEAN_SKELETON
                    bRtn = true;
#else
...
bool COcTree::RecDualThinEdge(UCHAR *pNode[2][2],
                              UINT64 n64Position[2][2],
                              int nLevel[2][2],
                              int dirEdge) {
    bool bRtn = false;
            ...
            switch (dirEdge) {
                case 0:
                    for (int i = 0; i < 2; i++)
                        for (int j = 0; j < 2; j++)
                            for (int k = 0; k < 2; k++)
                                Leaf(pNode[j][k], n64Position[j][k],
                                     nLevel[j][k], i, 1 - j, 1 - k,
                                     pNodePoint[i][j][k], n64Point[i][j][k],
                                     nPoint[i][j][k]);
                    if (RecDualThinPoint(pNodePoint, n64Point, nPoint))
                        bRtn = true;
                ...
                case 1:
                    for (int i = 0; i < 2; i++)
                        for (int j = 0; j < 2; j++)
                            for (int k = 0; k < 2; k++)
                                Leaf(pNode[k][i], n64Position[k][i],
                                     nLevel[k][i], 1 - i, j, 1 - k,
                                     pNodePoint[i][j][k], n64Point[i][j][k],
                                     nPoint[i][j][k]);
                    if (RecDualThinPoint(pNodePoint, n64Point, nPoint))
                        bRtn = true;
                ...
                case 2:
                    for (int i = 0; i < 2; i++)
                        for (int j = 0; j < 2; j++)
                            for (int k = 0; k < 2; k++)
                                Leaf(pNode[i][j], n64Position[i][j],
                                     nLevel[i][j], 1 - i, 1 - j, k,
                                     pNodePoint[i][j][k], n64Point[i][j][k],
                                     nPoint[i][j][k]);
                    if (RecDualThinPoint(pNodePoint, n64Point, nPoint))
                        bRtn = true;
        ...
        if (nSimpleTag != 0) {
            ...
            if (m_bCountOnly) {
                for (int i = 0; i < 2; i++) {
                    ...
                    dirFace = (dirEdge + 2) % 3;
                    if (GET_LAST_FIX(pNodeFace[0]) &&
                        GET_LAST_FIX(pNodeFace[1])) {
                        if (CheckStorageFace(pNodeFace)) {
                            GetStorageFace(nFace, dirFace, nFaceI, nFaceIndex);
                            nSimpleTag = GET_FACE_SIMPLE(
                                    ((LEAF_NODE *)pNodeFace[nFaceI])
                                            ->m_n64SimpleTag,
                                    nFaceIndex);
                            if (nSimpleTag != 3) {
                                SetObjectSimple(((LEAF_NODE *)pNodeFace[nFaceI])
                                                        ->m_n64SimpleTag,
                                                nFaceIndex, 2, nSimpleTag + 1);
                            }
                        }
                    }
                    for (int j = 0; j < 2; j++) {
                ...
                for (int i = 0; i < 2; i++) {
                    ...
                    if (CheckStorageFace(pNodeFace)) {
                        GetStorageFace(nFace, dirFace, nFaceI, nFaceIndex);
                        if (GET_FACE_SIMPLE(((LEAF_NODE *)pNodeFace[nFaceI])
                                                    ->m_n64SimpleTag,
                                            nFaceIndex) == 2) {
                            RETRACE_EDGE *pRetraceEdge = GetRetraceEdge(
                                    pNodeFace[nFaceI], nFaceIndex);
                            float fTempWeight = pRetraceEdge->m_fWeight +
                                                GetFaceArea(nFace[nFaceI]);
                ...
                if (nEdgeRI != -1) {
                    ...
                    SetObjectSimple(
                            ((LEAF_NODE *)pNodeUniqueFace)->m_n64SimpleTag,
                            nUniqueFaceIndex, 2, 0);
                    RETRACE_EDGE *pRetraceEdge =
                            GetRetraceEdge(pNodeUniqueFace, nUniqueFaceIndex);
                    GetEdgePos(n64Position, nLevel, dirEdge, nI, nJ, 0,
                               pRetraceEdge->m_n64PositionEdge);
...
bool COcTree::RecDualThinPoint(UCHAR *pNode[2][2][2],
                               UINT64 n64Position[2][2][2],
                               int nLevel[2][2][2]) {
    bool bRtn = false;
    ...
    if ((*(pNode[0][0][0]) & 0x3) == 0 || (*(pNode[0][0][1]) & 0x3) == 0 ||
        ...
        if (GET_LAST_FIX(pNode[0][0][0]) || GET_LAST_FIX(pNode[0][0][1]) ||
            ...
            for (int i = 0; i < 2; i++)
                for (int j = 0; j < 2; j++)
                    for (int k = 0; k < 2; k++)
                        Leaf(pNode[i][j][k], n64Position[i][j][k],
                             nLevel[i][j][k], 1 - i, 1 - j, 1 - k,
                             pNodePoint[i][j][k], n64Point[i][j][k],
                             nPoint[i][j][k]);
            if (RecDualThinPoint(pNodePoint, n64Point, nPoint)) bRtn = true;
        }
        ...
        if (nSimpleTag != 0) {
            if (m_bCountOnly) {
                for (int i = 0; i < 2; i++) {
                    ...
                    if (GET_LAST_FIX(pNodeEdge[0][0]) &&
                        GET_LAST_FIX(pNodeEdge[0][1]) &&
                        GET_LAST_FIX(pNodeEdge[1][0]) &&
                        GET_LAST_FIX(pNodeEdge[1][1])) {
                        if (CheckStorageEdge(pNodeEdge)) {
                            dirEdge = 0;
                            GetStorageEdge(nEdge, dirEdge, nEdgeI, nEdgeJ,
                                           nEdgeIndex);
                ...
                if (i < 2) {
                    ...
                    SetObjectSimple(
                            ((LEAF_NODE *)pNodeUniqueEdge)->m_n64SimpleTag,
                            nUniqueEdgeIndex, 1, 0);
                    bRtn = true;
                } else {
                    SetObjectSimple(pTempNode->m_n64SimpleTag, nIndex, 0, 1);
                }
...
void COcTree::DualCreateMST(float fThreshold, CString strSKTFile) {
    ...
    while (!heapFace.Empty()) {
        ...
        // PrintFaceInfo(cElement.m_pNode, cElement.m_n64Position,
        // cElement.m_nLevel, cElement.m_dirFace);
        GetStorageFace(cElement.m_nLevel, cElement.m_dirFace, nI, nIndex);
        if (GetFaceNeighborCell(cElement.m_pNode, cElement.m_n64Position,
                                cElement.m_nLevel, cElement.m_dirFace,
                                nCellIndex) ||
            cElement.m_fValue > fThreshold) {
            SetObjectSimple(((LEAF_NODE *)cElement.m_pNode[nI])->m_n64SimpleTag,
                            nIndex, 2, 1);
            SetObjectSimple(
                    ((LEAF_NODE *)cElement.m_pNode[nCellIndex])->m_n64SimpleTag,
                    0, 3, 0);
            ...
            for (int i = 0; i < 2; i++)
                for (dirFace = 0; dirFace < 3; dirFace++) {
                    UINT64 n64Search = LeafPosition(
                            n64BaseCell, nBaseCell,
                            CConstant::I_SHIFT_FP[CConstant::II_SHIFT_CF
                                                          [dirFace][1 - i]][0]);
                    GetFaceFromPos(n64Search, nBaseCell, dirFace, pNodeFace,
                                   n64Face, nFace);
            ...
            if (bOutput) {
                ...
                for (int i = 0; i < 2; i++) {
                    if (cElement.m_nLevel[i] == -1) {
                        n64Pos = FacePosition(cElement.m_n64Position,
                                              cElement.m_nLevel,
                                              cElement.m_dirFace);
                    } else {
                        n64Pos = NodePosition(cElement.m_n64Position[nI],
                                              cElement.m_nLevel[nI]);
                    }
                    fwrite(&n64Pos, sizeof(UINT64), 1, pFile);
                }
            }
...
void COcTree::RecPushFace(UCHAR *pNode[2],
                          UINT64 n64Position[2],
                          int nLevel[2],
                          int dirFace,
                          CFaceHeap &heapFace) {
    ...
    if (NOT_LEAF(pNode[0]) || NOT_LEAF(pNode[1])) {
            ...
            case 0:
                for (int i = 0; i < 2; i++)
                    for (int j = 0; j < 2; j++) {
                        ...
                        RecPushFace(pNodeFace, n64Face, nFace, dirFace,
                                    heapFace);
                    }
                break;
            case 1:
        ...
        if (CheckStorageFace(pNode)) {
            GetStorageFace(nLevel, dirFace, nI, nIndex);
            if (GET_FACE_SIMPLE(((LEAF_NODE *)pNode[nI])->m_n64SimpleTag,
                                nIndex) == 2) {
                SetObjectSimple(((LEAF_NODE *)pNode[nI])->m_n64SimpleTag,
                                nIndex, 2, 3);
                heapFace.Push(
                        pNode, n64Position, nLevel, dirFace,
                        GetFaceWeight(pNode, n64Position, nLevel, dirFace));
                // printf("\n\n# Push element, value is : %10.8f",
                // GetFaceWeight(pNode, n64Position, nLevel, dirFace));
...
void COcTree::DualGrow() {
    ...
    for (it = m_listGrowSeedFace.begin(); it != m_listGrowSeedFace.end();
         it++) {
        DualGrowByFace(*it);
    }
...
void COcTree::DualGrowByFace(CFaceHeapElement &cElement) {
    ...
    GetFacePos(cElement.m_n64Position, cElement.m_nLevel, cElement.m_dirFace,
               nI, 0, n64Face);
    dirEdge = (cElement.m_dirFace + 2) % 3;
    GetEdgeFromPos(n64Face, cElement.m_nLevel[nI], dirEdge, pNodeEdge, n64Edge,
                   nEdge);
...
void COcTree::RecDualGrowByEdge(UCHAR *pNode[2][2],
                                UINT64 n64Position[2][2],
                                int nLevel[2][2],
                                int dirEdge) {
    UCHAR *pNodeEdge[2][2];
    ...
    if (NOT_LEAF(pNode[0][0]) || NOT_LEAF(pNode[0][1]) ||
        ...
        int nI, nJ, nIndex;
        GetStorageEdge(nLevel, dirEdge, nI, nJ, nIndex);
        if (GET_EDGE_SIMPLE(((LEAF_NODE *)pNode[nI][nJ])->m_n64SimpleTag,
                            nIndex) != 3 &&
            !GET_EDGE_INSIDE(((LEAF_NODE *)pNode[nI][nJ])->m_n64Type, nIndex)) {
            SetObjectSimple(((LEAF_NODE *)pNode[nI][nJ])->m_n64SimpleTag,
                            nIndex, 1, 3);
            UINT64 n64Search;
            for (int i = 0; i < 2; i++) {
                GetEdgePos(n64Position, nLevel, dirEdge, nI, nJ, i, n64Search);
                GetPointFromPos(n64Search, MAX_LEVEL, pNodePoint, n64Point,
                                nPoint);
                GetStoragePoint(nPoint, nPointI, nPointJ, nPointK, nPointIndex);
                if (!GET_POINT_INSIDE(
                            ((LEAF_NODE *)pNodePoint[nPointI][nPointJ][nPointK])
                                    ->m_n64Type,
                            nPointIndex)) {
                    SetObjectSimple(
                            ((LEAF_NODE *)pNodePoint[nPointI][nPointJ][nPointK])
            ...
            int nEdgeRI = (int)GET_EDGE_RI(
                    ((LEAF_NODE *)pNode[nI][nJ])->m_n64Type, nIndex);
                ...
                if (pRetraceEdge->m_n64PositionEdge == n64EdgePosition) {
                    ...
                    if (GET_FACE_SIMPLE(((LEAF_NODE *)pNodeFace[nFaceI])
                        ...
                        SetObjectSimple(((LEAF_NODE *)pNodeFace[nFaceI])
                                                ->m_n64SimpleTag,
                                        nFaceIndex, 2, 3);
                        DualGrowByFace(CFaceHeapElement(pNodeFace, n64Face,
                                                        nFace, dirFace, 0.0f));
                    }
                } else {
